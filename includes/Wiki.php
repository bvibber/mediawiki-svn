...
class MediaWiki {
	...
	function checkInitialQueries( $title,$action,&$output,$request, $lang) {
		...
		if ( '' == $title && 'delete' != $action ) {
			...
			$wgArticleLanguage = $this->checkArticleLanguage(&$title, ':');//added by gkpr
	...
	function preliminaryChecks ( &$title, &$output, $request ) {
		...
		//$this->setVal( 'Language', $this->checkArticleLanguage(&$search)); //gkpr
		$wgArticleLanguage = empty($wgArticleLanguage) ? $this->checkArticleLanguage(&$search) : $wgArticleLanguage;//gkpr
	...
	function initializeSpecialCases ( &$title, &$output, $request ) {
		...
		$language = $title->mArticleLanguage; //gkpr
	...
	function checkArticleLanguage($input, $chartofind = ':') {
		...
		if ( ($pos = strpos($input, $chartofind))  && $pos == 2) {
			$language = strtolower(substr($input, 0, 2));
