...
class LqtView {
	...
	protected $languageGridAccessObject; /* Access object to Language Grid */
	...
	function __construct( &$output, &$article, &$title, &$user, &$request ) {
		...
		// Construct access object to language grid
	    $this->languageGridAccessObject = new LanguageGridAccessObject(); 
	}
	...
	*************************************************************/
	...
	/**
	 * modify to store post language into database by kadowaki
	 */
	static function postEditUpdates( $edit_type, $edit_applies_to, $edit_page, $article,
		...
		if ( $edit_type == 'reply' ) {
			...
			// store post language into database
			if ($postlang) {
				/** Insert thread language into DB */
				ThreadLanguage::create($thread->id(), $postlang);
			}
	...
	*************************/
	...
	function showThreadBody( $thread ) {
		...
		if ( $this->methodAppliesToThread( 'edit', $thread ) ) {
			...
			/* Translate posted body by LanguageGridAccessObject */
			$sourceBody = $this->showPostBody( $post, $oldid );
			$translatedBody = $this->languageGridAccessObject->translateBody($sourceBody, $thread);
			$html .= $translatedBody;
	...
	function threadSignature( $thread ) {
		...
		/* Display source language of thread body at UI of LiquidThreads. */
		$languageCode = $this->languageGridAccessObject->getSourceLanguage($thread->id());
		$signature .= Xml::element( 'span', null,
        wfMsg('multilang_lqt_post_language', 
	...
	function showThreadHeading( $thread ) {
		if ( $thread->hasDistinctSubject() ) {
			...
			/* Translate thread subject by LanguageGridAccessObject */
			$sourceSubject = $thread->subjectWithoutIncrement();
			$translatedSubject = $this->languageGridAccessObject->translateSubject($sourceSubject, $thread);
			$html = $this->output->parseInline($translatedSubject);
