...
if('fn'in $.wikiEditor.modules[module]&&'create'in $.wikiEditor.modules[module].fn){context.modules[module]={};$.wikiEditor.modules[module].fn.create(context,modules[module]);}}}}};context.evt={'change':function(event){event.data.scope='division';return true;},'delayedChange':function(event){return context.evt.change(event);}};context.fn={'trigger':function(name,event){if(typeof event=='undefined'){event={'type':'custom'};}
...
var newPos=t.node.nodeName=='#text'?pos+t.node.nodeValue.length:pos+1;if(!startContainer&&pos<=options.start&&options.start<newPos){startContainer=t.node;startOffset=options.start-pos;}
if(startContainer&&pos<=options.end&&options.end<newPos){endContainer=t.node;endOffset=options.end-pos;break;}
pos=newPos;t.goNext();}
sc=startContainer;ec=endContainer;start=startOffset;end=endOffset;}
if(!sc||!ec){return;}
...
var seekPos=range2.text.length;var t=context.fn.traverser(context.$content);var pos=0;while(t.node){if(t.node.nodeName!='#text'&&t.node.nodeName!='BR'){t.goNext();continue;}
var newPos=t.node.nodeName=='#text'?pos+t.node.nodeValue.length:pos+1;if(pos<=seekPos&&seekPos<newPos){break;}else{pos=newPos;t.goNext();}}
e=t.node;offset=seekPos-pos;if(!e)
return $([]);}
...
tokenArray.sort(function(a,b){return a.offset-b.offset||a.tokenStart-b.tokenStart;});context.fn.trigger('scan');},mark:function(context,division,tokens){var markers=context.modules.highlight.markers=[];context.fn.trigger('mark');markers.sort(function(a,b){return a.start-b.start||a.end-b.end;});var pos=0;var t=context.fn.traverser(context.$content);var i=0;var startNode=null;var nextDepth=0,startDepth=null;var lastTextNode=null,lastTextNodeDepth=null;while(i<markers.length&&t.node){if(t.node.nodeName!='#text'&&t.node.nodeName!='BR'){t.goNext();continue;}
var newPos=t.node.nodeName=='#text'?pos+t.node.nodeValue.length:pos+1;if(t.node.nodeName=='#text'){lastTextNode=t.node;lastTextNodeDepth=t.depth;}
if(!startNode&&markers[i].start>=pos&&markers[i].start<newPos){if(markers[i].start>pos){t.node=t.node.splitText(markers[i].start-pos);pos=markers[i].start;}
startNode=t.node;startDepth=t.depth;}
if(startNode&&startNode.nodeName=='BR'){startNode=t.node;startDepth=t.depth;}
if(startNode&&markers[i].end>pos&&markers[i].end<=newPos){if(markers[i].end<newPos){t.node.splitText(t.node.nodeValue.length-newPos+markers[i].end);newPos=markers[i].end;}
var endNode=t.node,endDepth=t.depth;if(endNode.nodeName=='BR'){endNode=lastTextNode;endDepth=lastTextNodeDepth;}
