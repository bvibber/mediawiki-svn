...
WikiEditorEngine = {
    ...
    keyHandler : function( evt ) {
	    if ( gecko ) {
		    ...
		    if ( ( evt.ctrlKey || evt.metaKey ) && evt.shiftKey
		            && charCode != 90 ) { // shortcuts =
											// ctrl||appleKey+shift+key!=z(undo)
			    WikiEditorEngine.shortcuts( charCode ? charCode : keyCode );
		    } else if ( ( completeEndingChars.indexOf( '|' + fromChar + '|' ) != -1 || completeChars
		            .indexOf( '|' + fromChar + '|' ) != -1 )
		            && WikiEditorEngine.autocomplete ) { // auto complete
			    if ( !WikiEditorEngine.completeEnding( fromChar ) )
				    WikiEditorEngine.complete( fromChar );
		    } else if ( chars.indexOf( '|' + charCode + '|' ) != -1
		            || keyCode == 13 ) { // syntax highlighting
			    top.setTimeout( function() {
				    WikiEditorEngine.syntaxHighlight( 'generic' );
			    }, 100 );
		    } else if ( keyCode == 9 || evt.tabKey ) { // snippets activation
														// (tab)
			    WikiEditorEngine.snippets( evt );
		    } else if ( keyCode == 46 || keyCode == 8 ) { // save to history
															// when delete or
															// backspace pressed
			    WikiEditorEngine.actions.history[WikiEditorEngine.actions
			            .next()] = editor.innerHTML;
		    } else if ( ( charCode == 122 || charCode == 121 || charCode == 90 )
		            && evt.ctrlKey ) { // undo and redo
			    ( charCode == 121 || evt.shiftKey ) ? WikiEditorEngine.actions
			            .redo() : WikiEditorEngine.actions.undo();
			    evt.preventDefault();
		    ...
		    if ( ( evt.ctrlKey || evt.metaKey ) && evt.shiftKey
		            && charCode != 90 ) { // shortcuts =
											// ctrl||appleKey+shift+key!=z(undo)
			    WikiEditorEngine.shortcuts( charCode ? charCode : keyCode );
		    } else if ( completeChars.indexOf( '|' + String
		            .fromCharCode( charCode ) + '|' ) != -1
		            && WikiEditorEngine.autocomplete ) { // auto complete
		    ...
		    if ( ( completeEndingChars.indexOf( '|' + fromChar + '|' ) != -1 || completeChars
		            .indexOf( '|' + fromChar + '|' ) != -1 )
		            && WikiEditorEngine.autocomplete ) { // auto complete
			    if ( !WikiEditorEngine.completeEnding( fromChar ) )
				    WikiEditorEngine.complete( fromChar );
		    } else if ( chars.indexOf( '|' + charCode + '|' ) != -1
		            || charCode == 13 ) { // syntax highlighting
			    WikiEditorEngine.syntaxHighlight( 'generic' );
		    }
	    }
    },
    ...
    metaHandler : function( evt ) {
	    ...
	    } else if ( ( keyCode == 122 || keyCode == 121 || keyCode == 90 )
	            && evt.ctrlKey ) { // undo and redo
		    ( keyCode == 121 || evt.shiftKey ) ? WikiEditorEngine.actions
		            .redo() : WikiEditorEngine.actions.undo();
		    evt.returnValue = false;
	    } else if ( keyCode == 34 || keyCode == 33 ) { // handle page up/down
														// for IE
		    self.scrollBy( 0, ( keyCode == 34 ) ? 200 : -200 );
		    evt.returnValue = false;
	    } else if ( keyCode == 46 || keyCode == 8 ) { // save to history when
														// delete or backspace
														// pressed
    ...
    split : function( code, flag ) {
	    if ( gecko ) {
		    ...
		    if ( flag == 'scroll' ) {
			    ...
			    code = code.substring( ini, end );
			    return code.substring( code.indexOf( '<P>' ), code
			            .lastIndexOf( '</P>' ) + 4 );
		    }
	    }
    },
    ...
    syntaxHighlight : function( flag ) {
	    if ( gecko ) {
		    ...
		    editor.innerHTML = this.actions.history[this.actions.next()] = ( flag == 'scroll' ) ? x
		            : o.split( z ).join( x );
		    if ( flag != 'init' )
			    this.findString();
	    } else if ( $.browser.opera ) {
    ...
    snippets : function( evt ) {
	    ...
	    for ( var i = 0; i < snippets.length; i++ ) {
		    if ( snippets[i].input == trigger ) {
			    ...
			    if ( !$.browser.msie ) {
				    evt.preventDefault(); // prevent the tab key from being
											// added
			    }
	    ...
	    for ( var i = 0; i < complete.length; i++ ) {
		    if ( complete[i].input == trigger ) {
			    ...
			    parent.setTimeout( function() {
				    WikiEditorEngine.syntaxHighlight( 'complete', pattern,
				            content )
    ...
    getCode : function() {
	    if ( gecko && !document.getElementsByTagName( 'pre' )[0]
	            || editor.innerHTML == '' )
		    editor = WikiEditorEngine.getEditor();
