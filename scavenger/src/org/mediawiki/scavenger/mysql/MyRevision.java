...
public class MyRevision implements Revision {
	 ...
	 */
	public MyRevision(ResultSet rs) throws SQLException {
	...
	private void loadFromDB() throws SQLException {
		...
		PreparedStatement stmt = dbc.prepareStatement(
				...
				"FROM revision, user WHERE rev_id = ? AND rev_user = user_id");
	 ...
	 */
	public MyRevision prevRevision() throws SQLException {
		loadFromDB();
		PreparedStatement stmt = dbc.prepareStatement(
		...
		MyRevision r = null;
		if (rs.next())
			r = new MyRevision(dbc, rs.getInt(1));
		rs.close();
		stmt.close();
		return r;
	}
	 ...
	 */
	public MyRevision nextRevision() throws SQLException {
		loadFromDB();
		PreparedStatement stmt = dbc.prepareStatement(
				"SELECT rev_id FROM revision WHERE rev_id > ? AND rev_page = ? " +
