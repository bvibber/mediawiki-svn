...
/* Copyright 2008 River Tarnell <river@wikimedia.org */
...
static void
err_output(int is_error, const char *fmt, va_list ap)
{
    time_t timevar;
    struct tm *lt;
...
#ifdef USE_PROJECTS
	if (getdefaultproj(actual_uname, &proj, nssbuf, sizeof(nssbuf)) == NULL) {
		log_err("failed to get default project (%s: %s)",
				actual_uname, strerror(errno));
		exit(113);
	}

	if (setproject(proj.pj_name, actual_uname, TASK_FINAL) != 0) {
		log_err("failed to set project (%s: %s)",
				actual_uname, strerror(errno));
		exit(114);
	}
#endif
