		...
		foreach( $link_tables as $link_table ) {
			...
			if( $showlinkedto ) {
				...
				if( $link_ns ) {
					...
					if ( $link_ns == NS_CATEGORY ) {
						$subconds = array( "cat_title" => $dbkey );
						$join_conds['category'] = array( 'INNER JOIN', 'cat_id=cl_target');
						$tables[] = 'category';
					} else $subconds = array( "{$pfx}_to" => $dbkey );
				} else {
				...
				} else if ( $link_table == 'categorylinks' ) {
					$subconds["rc_namespace"] = $link_ns;
					$subjoin = "rc_title = cat_title";
				} else {
					$subjoin = "rc_namespace = {$pfx}_namespace AND rc_title = {$pfx}_title";
				}
			}

			$subsql[] = $dbr->selectSQLText( array_merge( array( $link_table ), $tables ), $select, $conds + $subconds,
