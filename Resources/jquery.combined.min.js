...
$(this).trigger('scrollToPosition');});}});})(jQuery);(function($){$.fn.extend({encapsulateSelection:function(pre,peri,post){var e=this.jquery?this[0]:this;var selText;var isSample=false;if(document.selection&&document.selection.createRange){if(document.documentElement&&document.documentElement.scrollTop)
...
function checkSelectedText(){if(!selText){selText=peri;isSample=true;}else if(selText.charAt(selText.length-1)==' '){selText=selText.substring(0,selText.length-1);post+=' '}}}});})(jQuery);(function($){$.fn.extend({parseOutline:function(){return this.each(function(){var wikitext='\r\n'+$(this).val()+'\r\n';var headings=wikitext.match(/[\r\n][=]+[^\r\n]*[=]+[\r\n]/g);var outline=[];var offset=0;for(var h=0;h<headings.length;h++){text=headings[h];var position=wikitext.indexOf(text,offset);if(position>offset){offset=position;}else if(position==-1){continue;}
...
$(this).data('outline',outline)});},buildOutline:function(target){return this.each(function(){if(target.size()){var outline=$(this).data('outline');var level=1;for(var i=0;i<outline.length;i++){if(i>0){if(outline[i].level>outline[i-1].level){level++;}else if(outline[i].level<outline[i-1].level){level-=Math.max(1,outline[i-1].level-outline[i].level);}}
...
return sections;}
function buildList(textarea,structure){var list=$('<ul></ul>');for(i in structure){var item=$('<li></li>').append($('<a></a>').attr('href','#').addClass('section-'+structure[i].index).data('textbox',textarea).data('position',structure[i].position).click(function(event){$(this).data('textbox').scrollToPosition($(this).data('position')-1);event.preventDefault();}).text(structure[i].text));if(structure[i].sections!==undefined){item.append(buildList(textarea,structure[i].sections));}
list.append(item);}
return list;}
