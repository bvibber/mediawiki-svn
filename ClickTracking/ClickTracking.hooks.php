...
class ClickTrackingHooks {
	...
	public static function getEventIDFromName($event_name){
    	...
    	if($id_num === false){
    		$dbw = wfGetDB( DB_MASTER );
    		$dbw->insert('click_tracking_events', 
    	...
    	if($id_num === false){ return 0;}
    	return $id_num;
	}
	
	/**
	 * Track particular event
	 * @param $session_id unique session id for this editing sesion
	 ...
	 * @param $contribs_in_timespan number of contributions user has made in a given timespan
