...
$(this).trigger('scrollToPosition');});}});})(jQuery);(function($){$.fn.wikiEditor=function(){var imgPath=wgScriptPath+'/extensions/UsabilityInitiative/images/WikiEditor/toolbar/';var context=$(this).data('context');if(typeof context!=='undefined'){arguments=$.makeArray(arguments);if(arguments.length>0){var call=arguments.shift();if(call in context.api){context.api[call](arguments);}
return $(this).data('context',context);}
return $(this);}
context={'$textarea':$(this),'modules':{},'data':{}};$(this).wrap($('<div></div>').addClass('wikiEditor-ui')).wrap($('<div></div>').addClass('wikiEditor-ui-bottom')).wrap($('<div></div>').addClass('wikiEditor-ui-text'));context.$ui=$(this).parent().parent().parent();context.$ui.prepend($('<div></div>').addClass('wikiEditor-ui-top'));context.modules.toolbar={performAction:function(action){switch(action.type){case'encapsulate':var parts={'pre':'','peri':'','post':''};for(part in parts){if(part+'Msg'in action.options){parts[part]=gM(action.options[part+'Msg'],(action.options[part]||null));}else{parts[part]=(action.options[part]||'')}}
context.$textarea.encapsulateSelection(parts.pre,parts.peri,parts.post);break;default:break;}},addSection:function($section,section,sectionId){var useTool=function(){var tool=$(this).data('tool');if('type'in tool&&'action'in tool){switch(tool.type){case'button':case'link':context.modules.toolbar.performAction(tool.action);break;case'select':if('list'in tool&&$(this).val()in tool.list){context.modules.toolbar.performAction(tool.list[$(this).val()].action);}
...
if(skip){continue;}}
var label=msg(tools[tool],'label');switch(tools[tool].type){case'button':$group.append($('<input />').attr({'src':imgPath+tools[tool].icon,'alt':label,'title':label,'type':'image'}).addClass('tool').addClass('tool-'+tool).data('tool',tools[tool]).click(useTool));break;case'select':var $select=$('<select></select>').data('tool',tools[tool]).change(useTool).append($('<option></option>').text(label)).appendTo($group);for(option in tools[tool].list){$select.append($('<option></option>').text(msg(tools[tool].list[option],'label')).attr('value',option));}
break;}}}
...
if('rows'in pages[page]&&typeof pages[page].rows=='object'){for(row in pages[page].rows){var $row=$('<tr></tr>').appendTo($table);for(cell in pages[page].rows[row]){$('<td></td>').addClass(cell).attr('valign','top').append($('<span></span>').text(msg(pages[page].rows[row][cell],'content'))).appendTo($row);}}}
...
var $index=$('<div></div>').attr('class','index').appendTo($section);var $pages=$('<div></div>').attr('class','pages').appendTo($section);addPages($index,$pages,section.pages,sectionId);break;}},build:function(configuration,editorId){if('main'in configuration){context.modules.toolbar.addSection(context.modules.$toolbar,configuration.main,'main');}
...
$.eachAsync(sectionQueue,{bulk:0,loop:function(index,section){context.modules.toolbar.addSection(section.$section,section.tools,section.id);section.$section.removeClass('loading')}});},create:function(configuration){context.modules.$toolbar=$('<div></div>').addClass('wikiEditor-ui-toolbar');context.modules.toolbar.build(configuration);context.$ui.find('.wikiEditor-ui-top').append(context.modules.$toolbar);}};context.modules.toc={update:function(){context.modules.$toc.find('a').removeClass('currentSelection');var position=context.$textarea.getCaretPosition();var section=0;if(context.data.outline.length>0){if(!(position<context.data.outline[0].position-1)){while(section<context.data.outline.length&&context.data.outline[section].position-1<position){section++;}
section=Math.max(0,section);}
context.modules.$toc.find('a.section-'+section).addClass('currentSelection');}},build:function(configuration,editorId){var outline=[];var wikitext='\n'+context.$textarea.val()+'\n';var headings=wikitext.match(/\n={1,5}.*={1,5}(?=\n)/g);var offset=0;for(var h=0;h<headings.length;h++){text=headings[h];var position=wikitext.indexOf(text,offset);if(position>offset){offset=position;}else if(position==-1){continue;}
...
var level=1;var trunc=0;for(var i=0;i<outline.length;i++){if(i>0){if(outline[i].level>outline[i-1].level){level++;}else if(outline[i].level<outline[i-1].level){if(trunc<=1){level-=Math.max(1,outline[i-1].level-outline[i].level);}}
...
list.append(item);}
return list;}
var structure=buildStructure(outline);structure.unshift({'text':wgTitle,'level':1,'index':0,'position':0});context.modules.$toc.html(buildList(structure));context.data.outline=outline;},create:function(configuration){context.modules.$toc=$('<div></div>').addClass('wikiEditor-ui-toc');context.modules.toc.build(configuration);context.$ui.find('.wikiEditor-ui-bottom').append(context.modules.$toc);context.modules.$toc.height(context.$ui.find('.wikiEditor-ui-bottom').height());context.modules.$toc.css('width','12em');context.$ui.find('.wikiEditor-ui-text').css('marginRight','12em');context.modules.toc.build();context.modules.toc.update();context.$textarea.bind('keyup encapsulateSelection',function(event){var context=$(this).data('context');$(this).eachAsync({bulk:0,loop:function(){context.modules.toc.build();context.modules.toc.update();}});}).bind('mouseup scrollToPosition',function(event){var context=$(this).data('context');$(this).eachAsync({bulk:0,loop:function(){context.modules.toc.update();}});});}};context.api={addModule:function(){if(arguments.length>=1&&arguments[0].length>=1){var module=arguments[0][0];var configuration=(arguments[0][1]?arguments[0][1]:{});if(module in context.modules&&'create'in context.modules[module]){context.modules[module].create(configuration);}}}};$(this).data('context',context);context.$textarea.scrollToCaretPosition(0);if(arguments.length>0&&typeof arguments[0]=='object'){if('modules'in arguments[0]){for(module in arguments[0].modules){context.api.addModule([module,arguments[0].modules[module]]);}}}
return $(this);}})(jQuery);