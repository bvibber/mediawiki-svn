...
public class WikiConfiguration_frwiki extends WikiConfiguration {
	...
	public WikiConfiguration_frwiki() {
		...
		conceptTypeSensors.add( new HasCategoryLikeSensor<ConceptType>(ConceptType.PLACE, "^(Pays|Territoire|R\u00e9publique|Subdivision|Ville|Municipalit\u00e9s|Ocean)(_|$)", 0));
		...
		conceptTypeSensors.add( new HasTemplateLikeSensor<ConceptType>(ConceptType.PLACE, "^(Infobox_)?(Pays|Continent|Commune_)(_|$)", 0));
		
		conceptTypeSensors.add( new HasCategoryLikeSensor<ConceptType>(ConceptType.PERSON, "(^Homme$|^Femme$|^Naissance_en|D\u00e9c\u00e8s_en)", 0));

		conceptTypeSensors.add( new HasCategorySensor<ConceptType>(ConceptType.NAME, "Pr\u00e9nom"));
		conceptTypeSensors.add( new HasCategorySensor<ConceptType>(ConceptType.NAME, "Patronyme"));

		conceptTypeSensors.add( new HasTemplateLikeSensor<ConceptType>(ConceptType.TIME, "^(Ann\u00e9es|Portail_ann\u00e9es_\\d+|Portails_?I+er?_mill\u00e9naire(_av\\._J\\.-C\\.)?|Portails_d\u00e9cennies)$", 0));

		conceptTypeSensors.add( new HasTemplateLikeSensor<ConceptType>(ConceptType.LIFEFORM, "^Taxobox_", 0));
		//TODO: cooperations & organizations
		
		resourceTypeSensors.add( new HasTemplateLikeSensor<ResourceType>(ResourceType.BAD, "^Suppression[ _/]", 0));
		...
		resourceTypeSensors.add( new HasCategoryLikeSensor<ResourceType>(ResourceType.LIST, "^Liste(_|$)", 0));
