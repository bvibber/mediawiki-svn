...
var sc=selection.getRangeAt(0).startContainer;if(sc.nodeName=="#text")sc=sc.parentNode;return $(sc);}else{return $([]);}},'getContents':function(){var html;if($.browser.msie){var $c=$(context.$content.get(0).cloneNode(true));$c.find('p').each(function(){if($(this).html()==''){$(this).replaceWith('<p></p>');}});html=$c.html();}else{html=context.$content.html();}
...
var endNode=e.node;if(e.offset+1<e.length-1&&endNode.nodeName=='#text'){var newEndNode=endNode;var oldEndNode=endNode.splitText(e.offset+1);var subtracted=e.offset+1;var oldLength=e.length;var j,o;for(j=end-subtracted;j<end;j++){if(j in context.offsets){o=context.offsets[j];o.node=oldEndNode;o.length=subtracted;}}
for(j=end;j<end-subtracted+oldLength;j++){if(j in context.offsets){o=context.offsets[j];o.node=newEndNode;o.offset-=subtracted;o.length-=subtracted;o.lastTextNode=oldEndNode;}}}
...
return model;},getTemplateDisplayName:function(model){var tName=model.getName();if(tName in $.wikiEditor.modules.templateEditor.name_mappings){return tName+": "+model.getValue($.wikiEditor.modules.templateEditor.name_mappings[tName]);}else if(model.getValue("name")!=""){return tName+": "+model.getValue("name");}else if(model.getValue("Name")!=""){return tName+": "+model.getValue("Name");}
