...
context.$content.trigger('encapsulateSelection',[pre,options.peri,post,options.ownline,options.replace]);return context.$textarea;},'getCaretPosition':function(options){},'setSelection':function(options){var sel=context.$iframe[0].contentWindow.getSelection();sel.extend(startContainer,start);sel.collapseToStart();sel.extend(endContainer,end);},'scrollToCaretPosition':function(options){},'scrollToTop':function($element,force){var body=context.$content.closest('body');var y=$element.offset().top-context.$content.offset().top;if(force||y<body.scrollTop()||y>body.scrollTop()+body.height())
...
var outline=[];var h=0;context.$content.contents().add(context.$content.find('*')).each(function(){if(this.nodeName!='#text'&&!$(this).is('.wikiEditor-toc-header'))
return;var text=this.nodeValue;if($(this).is('div')){text=$(this).html();var prev=this.previousSibling;if(prev&&prev.nodeName=='#text'){var prevText=prev.nodeValue;while(prevText.substr(-1)=='='){prevText=prevText.substr(0,prevText.length-1);text='='+text;}
...
if(text!=$(this).html())
$(this).html(text);}
var match=text.match(/^(={1,6})(.+?)\1\s*$/);if(!match){if($(this).is('.wikiEditor-toc-header'))
$(this).replaceWith($(this).html());return;}
var div;if($(this).is('.wikiEditor-toc-header'))
div=$(this);else if($(this).is('div'))
div=$(this).addClass('wikiEditor-toc-header');else{div=$j('<div />').text(text).css('display','inline').addClass('wikiEditor-toc-header');$(this).replaceWith(div);}
