...
define('META_DATA_EXT', '.meta'); //file extension for cached Time metadata 
 ...
 class MvFLV extends FLV {
	...
	function playTimeReq($start_time_sec, $end_time_sec=null){
		...
		$this->getMetaData();
		$meta = & $this->compMetaData['keyframes'];
	...
	{		
		if(is_file($this->fname . META_DATA_EXT)){
			$this->compMetaData = unserialize(file_get_contents ($this->fname . META_DATA_EXT));			
		}else{
