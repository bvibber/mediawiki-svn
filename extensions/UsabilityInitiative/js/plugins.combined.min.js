...
$(this).data('expandableField-context',context);});return returnValue!==null?returnValue:$(this);};})(jQuery);(function($){$.highlightText={splitAndHighlight:function(node,pat){var patArray=pat.split(" ");for(var i=0;i<patArray.length;i++){if(patArray[i].length==0)continue;$.highlightText.innerHighlight(node,patArray[i]);}
...
context.fn.updateHistory(event.data.scope=='realchange');return true;},'cut':function(event){setTimeout(function(){context.$content.find('br').each(function(){if($(this).parent().is('body')){$(this).wrap($('<p></p>'));}});},100);return true;},'paste':function(event){var cursorPos=context.fn.getCaretPosition();var oldLength=context.fn.getContents().length;var positionFromEnd=oldLength-cursorPos[1];context.$content.find('*').addClass('wikiEditor');if($.layout.name!=='webkit'){context.$content.addClass('pasting');}
setTimeout(function(){context.$content.find('script,style,img,input,select,textarea,hr,button,link,meta').remove();var nodeToDelete=[];var pastedContent=[];var firstDirtyNode;var $lastDirtyNode;var elementAtCursor;if($.browser.msie&&!context.offsets){elementAtCursor=null;}else{elementAtCursor=context.fn.getOffset(cursorPos[0]);}
if(elementAtCursor==null||elementAtCursor.node==null){context.$content.prepend('<p class = wikiEditor></p>');firstDirtyNode=context.$content.children()[0];}else{firstDirtyNode=elementAtCursor.node;}
try{firstDirtyNode.parentNode;}catch(err){context.$content.prepend('<p class = wikiEditor></p>');firstDirtyNode=context.$content.children()[0];}
while(firstDirtyNode!=null){while(firstDirtyNode.parentNode&&firstDirtyNode.parentNode.nodeName!='BODY'&&!$(firstDirtyNode).hasClass('wikiEditor')){firstDirtyNode=firstDirtyNode.parentNode;}
...
if(firstDirtyNode.previousSibling!=null){$lastDirtyNode=$(firstDirtyNode.previousSibling);}else{$lastDirtyNode=$(firstDirtyNode);}
var cc=makeContentCollector($.browser,null);while(firstDirtyNode!=null){cc.collectContent(firstDirtyNode);cc.notifyNextNode(firstDirtyNode.nextSibling);nodeToDelete.push(firstDirtyNode);firstDirtyNode=firstDirtyNode.nextSibling;if($(firstDirtyNode).hasClass('wikiEditor')){break;}}
var ccData=cc.finish();pastedContent=ccData.lines;var pastedPretty='';for(var i=0;i<pastedContent.length;i++){pastedPretty=pastedContent[i].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\r?\n/g,'\\n');match=pastedContent[i].match(/^[\s]+[^\s]/);if(match!=null&&match.length>0){index=match[0].length;leadingSpace=match[0].replace(/[\s]/g,'&nbsp;');pastedPretty=leadingSpace+pastedPretty.substring(index,pastedPretty.length);}
if(!pastedPretty&&$.browser.msie&&i==0){continue;}
$newElement=$('<p class="wikiEditor pasted" ></p>');if(pastedPretty){$newElement.html(pastedPretty);}else{$newElement.html('<br class="wikiEditor">');}
$newElement.insertAfter($lastDirtyNode);$lastDirtyNode=$newElement;}
while(nodeToDelete.length>0){$deleteNode=$(nodeToDelete.pop());$deleteNode.remove();}
$selection=context.$content.find(':not(.wikiEditor)');if($selection.length==0){break;}else{firstDirtyNode=$selection.eq(0)[0];}}
context.$content.find('.wikiEditor').removeClass('wikiEditor');var newLength=context.fn.getContents().length;var newPos=newLength-positionFromEnd;context.fn.purgeOffsets();context.fn.setSelection({start:newPos,end:newPos});context.fn.scrollToCaretPosition();},0);return true;},'ready':function(event){if(context.$content){context.history.push({'html':context.$content.html(),'sel':context.fn.getCaretPosition()});}else{context.history.push({'html':'','sel':context.fn.getCaretPosition()});}
...
var e=null,offset=null;if($.browser.msie&&!context.$iframe[0].contentWindow.document.body){return null;}
if(context.$iframe[0].contentWindow.getSelection){var selection=context.$iframe[0].contentWindow.getSelection();if(selection.baseNode!==null){e=selection.getRangeAt(0).startContainer;offset=selection.getRangeAt(0).startOffset;}else{return null;}
...
var sc=selection.getRangeAt(0).startContainer;if(sc.nodeName=="#text")sc=sc.parentNode;return $(sc);}else if(context.$iframe[0].contentWindow.document.selection){var selection=context.$iframe[0].contentWindow.document.selection.createRange();return $(selection.parentElement());}},'getContents':function(){if(!context.$content){return'';}
var html;if($.browser.msie){var $c=$(context.$content.get(0).cloneNode(true));$c.find('p').each(function(){if($(this).html()==''){$(this).replaceWith('<p></p>');}});html=$c.html();}else{html=context.$content.html();}
...
wrapper=context.$content;context.fn.scrollToTop(wrapper,true);context.$textarea.textSelection('setSelection',{'start':0,'startContainer':wrapper});context.fn.highlightLine($(wrapper));$(this).addClass('current');setTimeout(function(){$.wikiEditor.modules.toc.fn.unhighlight(context)},1000);if(typeof $.trackAction!='undefined')
...
if(isPre)
_incrementFlag(state,'preMode');var oldListTypeOrNull=null;var nc=dom.nodeNumChildren(node);for(var i=0;i<nc;i++){var c=dom.nodeChild(node,i);if(browser.msie&&cls.indexOf('wikiEditor')>=0&&dom.nodeName(c)=='SPAN'&&dom.nodeAttr(c,'lang')==""){continue;}
...
if(browser.msie){}};cc.notifyNextNode=function(node){if((!node)||(isBlockElement(node)&&!_isEmpty(node))){_ensureColumnZero(null);}};var getSelectionStart=function(){return selStart;};var getSelectionEnd=function(){return selEnd;};cc.getLines=function(){return lines.textLines();};cc.finish=function(){lines.flush();var lineStrings=cc.getLines();if(lineStrings.length>0&&!lineStrings[lineStrings.length-1]){lineStrings.length--;}
var ss=getSelectionStart();var se=getSelectionEnd();function fixLongLines(){var lineLimit=2000;var buffer=10;var linesWrapped=0;var numLinesAfter=0;for(var i=lineStrings.length-1;i>=0;i--){var oldString=lineStrings[i];if(oldString.length>lineLimit+buffer){var newStrings=[];while(oldString.length>lineLimit){lengthToTake=lineLimit;newStrings.push(oldString.substring(0,lengthToTake));oldString=oldString.substring(lengthToTake);}
