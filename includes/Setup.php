...
# Extension initialization for extensions using the new Extension class
$wgExtensions = array();
if( isset($wgExtensionPaths) && ( is_array($wgExtensionPaths) || is_string($wgExtensionPaths) ) ) {
	if( is_string($wgExtensionPaths) ) {
		$wgExtensionPaths = array( $wgExtensionPaths );
	}
	array_unshift($wgExtensionPaths, "$IP/extensions");
} else {
	$wgExtensionPaths = array( "$IP/extensions" );
}
foreach( $wgLoadExtensions as $extension => $config ) {
	# Handle odd cases like $wgLoadExtensions[] = 'Extension Name';
	if( is_numeric($extension) && is_string($config) ) $extension = $config;
	
	if( !Extension::StartLoadExtension( $extension, $config ) ) {
		wfDebug($fname.'-extensions/class: Failed to load '.$extension);
	}
}
Extension::DoLoadExtensions(); // Actual loading, delayed to handle dependencies correctly
