...
WikiEditorEngine = {
	...
	initialize : function() {
		...
		if($.browser.msie){
			...
			window.attachEvent('onscroll', function() { if(!WikiEditorEngine.scrolling) setTimeout(function(){WikiEditorEngine.syntaxHighlight('scroll')},1)});
		}
		else{ //!$.browser.msie
			...
			window.addEventListener('scroll', function() { if(!WikiEditorEngine.scrolling) WikiEditorEngine.syntaxHighlight('scroll') }, false);
		}
	...
	if(gecko){
		...
		if((evt.ctrlKey || evt.metaKey) && evt.shiftKey && charCode!=90)  { // shortcuts = ctrl||appleKey+shift+key!=z(undo) 
		...
		else if( (completeEndingChars.indexOf('|'+fromChar+'|')!= -1 || completeChars.indexOf('|'+fromChar+'|')!=-1) && WikiEditorEngine.autocomplete) { // auto complete
			...
			top.setTimeout(function(){WikiEditorEngine.syntaxHighlight('generic');},100);
		}
			...
			(charCode==121||evt.shiftKey) ? WikiEditorEngine.actions.redo() :  WikiEditorEngine.actions.undo(); 
			evt.preventDefault();
		}
		...
		else if(completeChars.indexOf('|'+String.fromCharCode(charCode)+'|')!=-1 && WikiEditorEngine.autocomplete) { // auto complete
		...
		if( (completeEndingChars.indexOf('|'+fromChar+'|')!= -1 || completeChars.indexOf('|'+fromChar+'|')!=-1  )&& WikiEditorEngine.autocomplete) { // auto complete
			if(!WikiEditorEngine.completeEnding(fromChar))
			     WikiEditorEngine.complete(fromChar);
		}
	...
	metaHandler : function(evt) {
		...
		if(keyCode==9 || evt.tabKey) { 
			...
			(keyCode==121||evt.shiftKey) ? WikiEditorEngine.actions.redo() :  WikiEditorEngine.actions.undo(); 
			...
			window.clipboardData.setData('Text',window.clipboardData.getData('Text').replace(/\t/g,'\u2008'));
		 	top.setTimeout(function(){WikiEditorEngine.syntaxHighlight('paste');},10);
		}
	...
	complete : function(trigger) {
		...
		for (var i=0; i<complete.length; i++) {
			if(complete[i].input == trigger) {
				...
				parent.setTimeout(function () { WikiEditorEngine.syntaxHighlight('complete',pattern,content)},0); // wait for char to appear on screen
			}
		}
	},
...
//g
	actions : {
		...
		undo : function() {
			...
			this.pos --;
			if(typeof(this.history[this.pos])=='undefined') this.pos++;  //in gecko it was pos ++ with a space. necessary?
			editor.innerHTML = this.history[this.pos];
			if(gecko && editor.innerHTML.indexOf(cc)>-1) editor.innerHTML+=cc;
...
Language={};
if($.browser.msie){
	window.attachEvent('onload', function() { WikiEditorEngine.initialize('new');});
}
else if(!$.browser.msie){
	window.addEventListener('load', function() { WikiEditorEngine.initialize('new'); }, true);
}
