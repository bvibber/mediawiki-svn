		...
		function search($searchText) {
			...
			if ($withinWords) {
				if ($languageId != 0 && $languageName != "")
					$wgOut->addHTML('<h1>' . wfMsgExt('datasearch_match_words_lang', array( 'parse', 'replaceafter') , $languageName, $searchText) . '</h1>');
				else
					$wgOut->addHTML('<h1>' . wfMsgExt('datasearch_match_words', array( 'parse', 'replaceafter') , $searchText) . '</h1>');
