...
class SpecialConnect extends SpecialPage {
	...
	static private $availableUserUpdateOptions = array(
		'fullname', 'nickname', 'email', 'language', 'timecorrection'
	);
		...
		case 'ChooseName':
			...
			switch ( $choice ) {
				...
				case 'full':
					...
					$username = FBConnectUser::getOptionFromInfo(
						$choice . 'name',
	...
	private function alreadyLoggedIn() {
		...
		$wgOut->addHTML(
			'<fb:login-button' . FBConnect::getPermissionsAttribute() .
	...
	private function chooseNameForm( $messagekey = 'fbconnect-chooseinstructions' ) {
		...
		foreach ( array( 'nick', 'first', 'full' ) as $option ) {
			...
			if ( $nickname && $this->userNameOK( $nickname ) ) {
				$wgOut->addHTML(
					'<tr><td class="mw-label"><input name="wpNameChoice" type="radio" value="' .
		...
		$wgOut->addHTML(
			'<tr><td class="mw-label"><input name="wpNameChoice" type="radio" value="auto" ' .
	...
	private function connectForm() {
		...
		$wgOut->addHTML(
			'<h2>' . wfMsg( 'fbconnect' ) . '</h2>
			<div>' . wfMsgExt( 'fbconnect-intro', array( 'parse', 'content' ) ) .
			'<br />' . wfMsg( 'fbconnect-click-to-login', $wgSitename ) . '
			<fb:login-button size="large" background="black" length="long"' .
				FBConnect::getPermissionsAttribute() . FBConnect::getOnLoginAttribute() .
				'></fb:login-button>
			</div>'
		);
	}
