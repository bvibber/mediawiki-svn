...
//	global $wfTimeStamp;
	...
	$poptions = new ParserOptions;
        ...
        foreach ( $params AS $paramString ){
            ...
            // not sure this is necessary, or even would work
            $param[0] = trim( $param[0] );
            $param[1] = trim( $param[1] );
            
            switch ( $param[0] ){
                case 'category':
                    $title = Title::newFromText( $parser->transformMsg( $param[1], $poptions ) );
		    ...
		    if ( is_object( $title ) )
                    ...
                    break;
                case 'namespace':
		    if ( $param[1] == intval( $param[1] ) ){
			$this->params['iNameSpace'] = intval( $param[1] );
			...
			$ns = $wgContLang->getNsIndex( $param[1] );
			if ( null !== $ns ){
			    ...
			    $this->params['nameSpace'] = true;
                    ...
                    break;
                case 'count':
                    if ( ( $this->wgDPLminResultCount < $param[1] ) && ( $param[1] < $this->wgDPLmaxResultCount ) ){
                            $this->params['count'] = intval( $param[1] );
                    }
                    break;
                case 'order';
                    switch ( $param[1] ){
			...
			default:
                ...
                case 'suppresserrors':
                    ...
                    if ( 'true' == $param[1] ) $this->params['suppressErrors'] = true;
                    break;
                case 'usenamespace':
                    // note: if previously set to false, remains false. Malformed does not reset to true.
                    if ( 'false' == $param[1] ) $this->params['useNameSpace'] = false;
                    break;
                case 'usecurid':
                    // note: if previously set to true, remains true. Malformed does not reset to false.
                    if ( 'true' == $param[1] ) $this->params['useCurId'] = true;
                    break;
                case 'feed':
...
<?php
	...
	?>
    <url>
	<loc><?php print $item->getUrl() ?></loc>
	<news:news>
	    <news:publication_date><?php print $item->getPubDate() ?></news:publication_date>
	    <?php if( $item->getKeywords() ){
		echo '<news:keywords>' . $item->getKeywords() . "</news:keywords>\n";
	    }
?>	</news:news>
<?php	 if( $item->getLastMod() ){ ?>	<lastmod><?php print $item->getLastMod(); ?></lastmod>
<?php }?>
<?php	 if( $item->getPriority() ){ ?>	<priority><? print $item->getPriority(); ?></priority><?php }?>
    </url>
<?php	
    }
