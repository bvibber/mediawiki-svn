...
return sections;}
function buildList(textarea,structure){var list=$('<ul></ul>');for(i in structure){var item=$('<li></li>').append($('<a></a>').attr('href','#').addClass('section-'+structure[i].index).data('textbox',textarea).data('position',structure[i].position).click(function(event){$(this).data('textbox').scrollToCaretPosition($(this).data('position'));event.preventDefault();}).text(structure[i].text));if(structure[i].sections!==undefined){item.append(buildList(textarea,structure[i].sections));}
list.append(item);}
return list;}
...
target.find('a').removeClass('currentSelection');target.find('a.section-'+section).addClass('currentSelection');});}});})(jQuery);