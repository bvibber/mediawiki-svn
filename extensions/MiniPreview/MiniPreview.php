...
$wgMiniPreviewMessagesInitialized = false;
...
#$messages['minipreview_total_files_in_gallery']= '';

$wgHooks['LoadAllMessages'][] = 'efInjectMiniPreviewMessages';
$wgHooks['ImageOpenShowImageInlineBefore'][] = 'efMiniPreviewShow';


function efMiniPreview () {
...
function efMiniPreviewShow ( &$imagePage, &$output )  {
  ...
  $data = wfMiniPreviewMessage ( 'minipreview-ignore_categories' ) ;
  $data = explode ( "\n" , $data ) ;
...
function wfMiniPreviewGetThumbnail ( $entry , &$image_data ) {
  ...
  if ( $entry->id != 0 ) { # Existing file
    ...
    $ret .= wfMiniPreviewMessage ( 'minipreview-no_more_files_here' ) ;
  }
...
/**
* inject messages used by MiniPreview into the message cache
*/
function efInjectMiniPreviewMessages() {
	wfMiniPreviewMessage(false);
	return true;
}
...
function wfMiniPreviewMessage( $msg , $args = array() ) {
  global $wgMiniPreviewMessagesInitialized;
  $wgMiniPreviewMessagesInitialized = false;
  global $wgMessageCache;
  if ( !$initialized ) {
    $wgMessageCache->addMessages( wfMiniPreviewLoadMessages() );
    $initialized = true;
  }
  if ( $msg === false ) {
    return null;
