	...
	static function getNotifyText() {
		...
		if ( $db->numRows( $res ) > 0 ) {
			...
			while ( $row = $db->fetchObject( $res ) ) {
				...
				$result .= '<span id="EditConflict_' . $row->ns_user_rev_id . '">(' . $skin->makeKnownLinkObj( $dest_title, 'rev. ' . $row->ns_user_rev_id . '', 'oldid=' . $row->ns_user_rev_id ) . '&nbsp;<span class="closelink" title="Закрыть предупреждение" onclick="EditConflict.clearRevId(' . $row->ns_user_rev_id . ');">&#8251;</span>' . ')</span> ';
			}
			$result .= '</li>';
		}
		return $result;
