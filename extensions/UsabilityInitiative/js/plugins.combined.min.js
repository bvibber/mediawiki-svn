...
$(this).trigger('scrollToPosition');});}});})(jQuery);RegExp.escape=function(s){return s.replace(/([.*+?^${}()|\/\\[\]])/g,'\\$1');};(function($){$.wikiEditor.modules.dialogs={api:{open:function(context,data){if(data.dialog in $.wikiEditor.modules.dialogs.modules){$('#'+$.wikiEditor.modules.dialogs.modules[data.dialog].id).dialog('open');}}},fn:{create:function(context,config){if('modules'in config){for(module in config.modules){$.wikiEditor.modules.dialogs.modules[module]=config.modules[module];}}
for(module in $.wikiEditor.modules.dialogs.modules){var module=$.wikiEditor.modules.dialogs.modules[module];if($('#'+module.id).size()==0){var configuration=module.dialog;configuration.bgiframe=true;configuration.autoOpen=false;configuration.modal=true;configuration.title=gM(module.titleMsg);for(msg in configuration.buttons){configuration.buttons[gM(msg)]=configuration.buttons[msg];delete configuration.buttons[msg];}
$('<div /> ').attr('id',module.id).html(module.html).data('context',context).appendTo($('body')).each(module.init).dialog(configuration);}}}},modules:{'insert-link':{titleMsg:'edittoolbar-tool-link-title',id:'edittoolbar-link-dialog',html:'<div id="edittoolbar-link-tabs"><ul><li><a href="#edittoolbar-link-dialog-tab-int" rel="edittoolbar-tool-link-int"></a></li><li><a href="#edittoolbar-link-dialog-tab-ext" rel="edittoolbar-tool-link-ext"></a></li></ul><div id="edittoolbar-link-dialog-tab-int"><form><label for="edittoolbar-link-int-target" rel="edittoolbar-tool-link-int-target"></label> <input type="text" id="edittoolbar-link-int-target" /> <div id="edittoolbar-link-int-target-status" style="display: inline;"></div><br /><label for="edittoolbar-link-int-text" rel="edittoolbar-tool-link-int-text"></label> <input type="text" id="edittoolbar-link-int-text" /></form></div><div id="edittoolbar-link-dialog-tab-ext"><form><label for="edittoolbar-link-ext-target" rel="edittoolbar-tool-link-ext-target"></label> <input type="text" id="edittoolbar-link-ext-target" /><br /><label for="edittoolbar-link-ext-text" rel="edittoolbar-tool-link-ext-text"></label> <input type="text" id="edittoolbar-link-ext-text" /></form></div></div>',init:function(){$(this).find('[rel]').each(function(){$(this).text(gM($(this).attr('rel')));});$('#edittoolbar-link-tabs').tabs();$('#edittoolbar-link-int-target').bind('keypress paste',function(){if($('#edittoolbar-link-int-text').data('untouched'))
setTimeout("$( '#edittoolbar-link-int-text' ).val( $( '#edittoolbar-link-int-target' ).val() );",0);});$('#edittoolbar-link-int-text').bind('keypress paste',function(){$(this).data('untouched',false);});$('#edittoolbar-link-ext-target').val('http://');var existsImg=$.wikiEditor.modules.toolbar.imgPath+'insert-link-exists.png';var notexistsImg=$.wikiEditor.modules.toolbar.imgPath+'insert-link-notexists.png';var invalidImg=$.wikiEditor.modules.toolbar.imgPath+'insert-link-invalid.png';var loadingImg=$.wikiEditor.modules.toolbar.imgPath+'loading.gif';var existsMsg=gM('edittoolbar-tool-link-int-target-status-exists');var notexistsMsg=gM('edittoolbar-tool-link-int-target-status-notexists');var invalidMsg=gM('edittoolbar-tool-link-int-target-status-invalid');var loadingMsg=gM('edittoolbar-tool-link-int-target-status-loading');$('#edittoolbar-link-int-target-status').append($('<img />').attr({'id':'edittoolbar-link-int-target-status-exists','src':existsImg,'alt':existsMsg,'title':existsMsg})).append($('<img />').attr({'id':'edittoolbar-link-int-target-status-notexists','src':notexistsImg,'alt':notexistsMsg,'title':notexistsMsg})).append($('<img />').attr({'id':'edittoolbar-link-int-target-status-invalid','src':invalidImg,'alt':invalidMsg,'title':invalidMsg})).append($('<img />').attr({'id':'edittoolbar-link-int-target-status-loading','src':loadingImg,'alt':loadingMsg,'title':loadingMsg})).data('cache',{}).children().hide();function updateExistence(target){function updateWidget(status){$('#edittoolbar-link-int-target-status').children().hide();$('#edittoolbar-link-int-target-status-'+status).show();}
var request=$('#edittoolbar-link-int-target-status').data('request');if(request)
request.abort();var target=$('#edittoolbar-link-int-target').val();var cache=$('#edittoolbar-link-int-target-status').data('cache');if(cache[target]){updateWidget(cache[target]);return;}
if(target==''){$('#edittoolbar-link-int-target-status').children().hide();return;}
if(target.indexOf('|')!=-1){updateWidget('invalid');return;}
updateWidget('loading');var request=$.ajax({url:wgScriptPath+'/api.php',dataType:'json',data:{'action':'query','indexpageids':'','titles':target,'format':'json'},success:function(data){var page=data.query.pages[data.query.pageids[0]];var status='exists';if(typeof page.missing!='undefined')
status='notexists';else if(typeof page.invalid!='undefined')
status='invalid';cache[target]=status;updateWidget(status);}});$('#edittoolbar-link-int-target-status').data('request',request);}
$('#edittoolbar-link-int-target').bind('keypress paste',function(){if(typeof $(this).data('timerID')!='undefined')
clearTimeout($(this).data('timerID'));var timerID=setTimeout(updateExistence,250);$(this).data('timerID',timerID);}).change(function(){if(typeof $(this).data('timerID')!='undefined')
clearTimeout($(this).data('timerID'));updateExistence();});},dialog:{width:550,buttons:{'edittoolbar-tool-link-insert':function(){function escapeInternalText(s){return s.replace(/(]{2,})/g,'<nowiki>$1</nowiki>');}
...
var insertText='';var whitespace=['',''];switch($('#edittoolbar-link-tabs').tabs('option','selected')){case 0:if($('#edittoolbar-link-int-target-status-invalid').is(':visible')){alert(gM('edittoolbar-tool-link-int-invalid'));return;}
var target=$('#edittoolbar-link-int-target').val();var text=$('#edittoolbar-link-int-text').val();whitespace=$('#edittoolbar-link-dialog-tab-int').data('whitespace');if(target==text)
insertText='[['+target+']]';else
insertText='[['+target+'|'+escapeInternalText(text)+']]';break;case 1:var target=$('#edittoolbar-link-ext-target').val();var text=$('#edittoolbar-link-ext-text').val();var escTarget=escapeExternalTarget(target);var escText=escapeExternalText(text);whitespace=$('#edittoolbar-link-dialog-tab-ext').data('whitespace');if(escTarget==escText)
insertText=escTarget;else if(text=='')
insertText='['+escTarget+']';else
insertText='['+escTarget+' '+escText+']';break;}
insertText=whitespace[0]+insertText+whitespace[1];$.wikiEditor.modules.toolbar.fn.doAction($(this).data('context'),{type:'replace',options:{pre:insertText}},$(this));$(this).dialog('close');},'edittoolbar-tool-link-cancel':function(){$(this).dialog('close');}},open:function(){var selection=$(this).data('context').$textarea.getSelection();$('#edittoolbar-link-dialog-tab-int').data('whitespace',['','']);$('#edittoolbar-link-dialog-tab-ext').data('whitespace',['','']);if(selection!=''){var inttext,inttarget,exttext,exttarget;var matches;var tab=-1;if((matches=selection.match(/^(\s*)\[\[([^\]\|]+)(\|([^\]\|]*))?\]\](\s*)$/))){inttarget=matches[2];inttext=(matches[4]?matches[4]:matches[2]);tab=0;$('#edittoolbar-link-dialog-tab-int').data('whitespace',[matches[1],matches[5]]);}else if((matches=selection.match(/^(\s*)\[([^\] ]+)( ([^\]]+))?\](\s*)$/))){exttarget=matches[2];exttext=(matches[4]?matches[4]:'');tab=1;$('#edittoolbar-link-dialog-tab-ext').data('whitespace',[matches[1],matches[5]]);}else{inttarget=inttext=exttext=selection;exttarget='http://';}
if(typeof inttext!='undefined')
$('#edittoolbar-link-int-text').val(inttext).change();if(typeof inttarget!='undefined')
$('#edittoolbar-link-int-target').val(inttarget).change();if(typeof exttext!='undefined')
$('#edittoolbar-link-ext-text').val(exttext).change();if(typeof exttarget!='undefined')
$('#edittoolbar-link-ext-target').val(exttarget).change();if(tab!=-1)
$('#edittoolbar-link-tabs').tabs('select',tab);}
$('#edittoolbar-link-int-text').data('untouched',$('#edittoolbar-link-int-text').val()==$('#edittoolbar-link-int-target').val());}}},'insert-table':{titleMsg:'edittoolbar-tool-table-title',id:'edittoolbar-table-dialog',html:'<form><fieldset><legend rel="edittoolbar-tool-table-dimensions"></legend>'+'<table><tr><td class="label"><label for="edittoolbar-table-dimensions-columns"'+' rel="edittoolbar-tool-table-dimensions-columns"></label></td>'+'<td><input type="text" id="edittoolbar-table-dimensions-columns" size="2" /></td>'+'<td class="label"><label for="edittoolbar-table-dimensions-rows"'+' rel="edittoolbar-tool-table-dimensions-rows"></label></td>'+'<td><input type="text" id="edittoolbar-table-dimensions-rows" size="2" /></td></tr>'+'<tr><td class="label"><label for="edittoolbar-table-dimensions-headercolumns"'+' rel="edittoolbar-tool-table-dimensions-headercolumns"></label></td>'+'<td><input type="text" id="edittoolbar-table-dimensions-headercolumns" size="2" /></td>'+'<td class="label"><label for="edittoolbar-table-dimensions-headerrows"'+' rel="edittoolbar-tool-table-dimensions-headerrows"></label></td>'+'<td><input type="text" id="edittoolbar-table-dimensions-headerrows" size="2" /></td></tr>'+'</table></fieldset></form>',init:function(){$(this).find('[rel]').each(function(){$(this).text(gM($(this).attr('rel')));});},dialog:{width:350,buttons:{'edittoolbar-tool-table-insert':function(){var rows=parseInt($('#edittoolbar-table-dimensions-rows').val());var cols=parseInt($('#edittoolbar-table-dimensions-columns').val());var hrows=parseInt($('#edittoolbar-table-dimensions-headerrows').val());var hcols=parseInt($('#edittoolbar-table-dimensions-headercolumns').val());var table="{|\n";for(var r=0;r<rows;r++){table+="|-\n";for(var c=0;c<cols;c++){var isHeader=(r<hrows||c<hcols);var delim=isHeader?'!':'|';if(c>0)
delim+=delim;table+=delim+' '+
gM(isHeader?'edittoolbar-tool-table-example-header':'edittoolbar-tool-table-example',[r+1,c+1])+' ';}
table=table.substr(0,table.length-1)+"\n";}
table+="|}";$.wikiEditor.modules.toolbar.fn.doAction($(this).data('context'),{type:'encapsulate',options:{pre:table,ownline:true}},$(this));$(this).dialog('close');},'edittoolbar-tool-table-cancel':function(){$(this).dialog('close');}}}},'search-and-replace':{titleMsg:'edittoolbar-tool-replace-title',id:'edittoolbar-replace-dialog',html:'<form><fieldset><label for="edittoolbar-replace-search" rel="edittoolbar-tool-replace-search"></label> <input type="text" id="edittoolbar-replace-search" /><br /><label for="edittoolbar-replace-replace" rel="edittoolbar-tool-replace-replace"></label> <input type="text" id="edittoolbar-replace-replace" /><br /><input type="checkbox" id="edittoolbar-replace-case" /><label for="edittoolbar-replace-case" rel="edittoolbar-tool-replace-case"></label><br /><input type="checkbox" id="edittoolbar-replace-regex" /><label for="edittoolbar-replace-regex" rel="edittoolbar-tool-replace-regex"></label><br /><input type="checkbox" id="edittoolbar-replace-all" /><label for="edittoolbar-replace-all" rel="edittoolbar-tool-replace-all"></label></fieldset></form>',init:function(){$(this).find('[rel]').each(function(){$(this).text(gM($(this).attr('rel')));});},dialog:{width:350,buttons:{'edittoolbar-tool-replace-button':function(){var searchStr=$('#edittoolbar-replace-search').val();var replaceStr=$('#edittoolbar-replace-replace').val();var flags='';if(!$('#edittoolbar-replace-case').is(':checked'))
flags+='i';if($('#edittoolbar-replace-all').is(':checked'))
flags+='g';if(!$('#edittoolbar-replace-regex').is(':checked'))
searchStr=RegExp.escape(searchStr);var regex=new RegExp(searchStr,flags);var $textarea=$(this).data('context').$textarea;if(!$textarea.val().match(regex))
alert(gM('edittoolbar-tool-replace-nomatch'));else
$textarea.val($textarea.val().replace(regex,replaceStr));},'edittoolbar-tool-replace-close':function(){$(this).dialog('close');}}}}}};})(jQuery);(function($){$.wikiEditor={'modules':{},'instances':[],'supportedBrowsers':{'ltr':{'msie':7,'firefox':2,'opera':9,'safari':3,'chrome':1,'camino':1},'rtl':{'msie':8,'firefox':2,'opera':9,'safari':3,'chrome':1,'camino':1}}};$.wikiEditor.isSupportKnown=function(){return(function(supportedBrowsers){return $.browser.name in supportedBrowsers;})($.wikiEditor.supportedBrowsers[$('body.rtl').size()?'rtl':'ltr']);};$.wikiEditor.isSupported=function(){return(function(supportedBrowsers){return $.browser.name in supportedBrowsers&&$.browser.versionNumber>=supportedBrowsers[$.browser.name];})($.wikiEditor.supportedBrowsers[$('body.rtl').size()?'rtl':'ltr']);};$.fn.wikiEditor=function(){var context=$(this).data('context');if(typeof context!=='undefined'){arguments=$.makeArray(arguments);if(arguments.length>0){var call=arguments.shift();if(call in context.api){context.api[call](context,arguments[0]==undefined?{}:arguments[0]);}
...
context.modules.$toolbar.find(target).remove();}}},fn:{autoMsg:function(object,property){if(typeof property=='array'){for(i in property){if(property[i]in object){property=property[i];break;}else if(property[i]+'Msg'in object){property=property[i]+'Msg';break;}}}
if(property in object){return object[property];}else if(property+'Msg'in object){return gM(object[property+'Msg']);}else{return'';}},create:function(context,config){if('$toolbar'in context.modules){return;}
...
break;case'dialog':context.$textarea.wikiEditor('open',{'dialog':action.module});break;default:break;}},buildGroup:function(context,id,group){var $group=$('<div />').attr({'class':'group group-'+id,'rel':id});var label=$.wikiEditor.modules.toolbar.fn.autoMsg(group,'label');if(label){$group.append('<div class="label">'+label+'</div>')}
...
return $page;},buildHeading:function(context,headings){var html='<tr>';for(heading in headings){html+='<th>'+$.wikiEditor.modules.toolbar.fn.autoMsg(headings[heading],['html','text'])+'</th>';}
