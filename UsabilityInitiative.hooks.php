<?php
/**
 * Hooks for Usability Initiative extensions
 *
 * @file
 * @ingroup Extensions
 */

class UsabilityInitiativeHooks {

	/* Static Members */

	private static $doOutput = false;
	private static $messages = array();
	private static $variables = array();
	private static $literalVariables = array();
	private static $styles = array();
	private static $styleFiles = array(
		'base_sets' => array(
			'raw' => array(
				array( 'src' => 'css/suggestions.css', 'version' => 12, 'class' => "mw.style.usabilitySugest" ),
				array( 'src' => 'css/vector.collapsibleNav.css', 'version' => 9, 'class' => "mw.style.vectorCollapsibleNav" ),
				array( 'src' => 'css/vector.footerCleanup.css', 'version' => 2 , 'class' => "mw.style.vectorFooterCleanup" ),
				array( 'src' => 'css/wikiEditor.css', 'version' => 13, 'class'=> "mw.style.wikiEditor" ),
				array( 'src' => 'css/wikiEditor.dialogs.css', 'version' => 29, 'class' => "mw.style.wikiEditorDialogs" ),
				array( 'src' => 'css/wikiEditor.preview.css', 'version' => 1 , 'class' => "mw.style.wikiEditorPreview" ),
				array( 'src' => 'css/wikiEditor.toc.css', 'version' => 31 , 'class' => "mw.style.wikiEditorToc" ),
				array( 'src' => 'css/wikiEditor.toolbar.css', 'version' => 18, 'class' => "mw.style.wikiEditorToolbar" ),
				array( 'src' => 'css/vector/jquery-ui-1.7.2.css', 'version' => '1.7.2y', 'class' => "mw.style.vectorJquery-ui" ),
			),
			'combined' => array(
				array( 'src' => 'css/combined.css', 'version' => 92 ),
				array( 'src' => 'css/vector/jquery-ui-1.7.2.css', 'version' => '1.7.2y' ),
			),
			'minified' => array(
				array( 'src' => 'css/combined.min.css', 'version' => 92 ),
				array( 'src' => 'css/vector/jquery-ui-1.7.2.css', 'version' => '1.7.2y' ),
			),
		)
	);
	private static $scripts = array();
	private static $scriptFiles = array(
		'tests' => array(
			array( 'src' => 'js/tests/wikiEditor.toolbar.js', 'version' => 0 )
		),
		'modules' => array(
			'raw' => array(),
			'combined' => array(),
			'minified' => array()
		),
		// Core functionality of extension
		'base_sets' => array(
			'raw' => array(
				// Common UsabilityInitiative funtions
				//array( 'src' => 'js/usability.js', 'version' => 1 ),

				// These scripts can be pulled from core once the js2 is merged
				array( 'src' => 'js/js2stopgap/ui.core.js', 'version' => 1, 'class' => 'j.ui'),
				array( 'src' => 'js/js2stopgap/ui.datepicker.js', 'version' => 1, 'class' => 'j.fn.datePicker' ),
				array( 'src' => 'js/js2stopgap/ui.dialog.js', 'version' => 1, 'class' => 'j.ui.dialog' ),
				array( 'src' => 'js/js2stopgap/ui.draggable.js', 'version' => 1, 'class' => 'j.ui.draggable' ),
				array( 'src' => 'js/js2stopgap/ui.resizable.js', 'version' => 1, 'class' => 'j.ui.resizable' ),
				array( 'src' => 'js/js2stopgap/ui.tabs.js', 'version' => 1, 'class' => 'j.ui.tabs' ),

				// Core functionality of extension scripts
				array( 'src' => 'js/plugins/jquery.async.js', 'version' => 3, 'class' => 'j.whileAsync' ),
				array( 'src' => 'js/plugins/jquery.autoEllipsis.js', 'version' => 7, 'class'=> 'j.fn.autoEllipsis' ),
				array( 'src' => 'js/plugins/jquery.browser.js', 'version' => 6 , 'class' => 'j.browserTest' ),
				array( 'src' => 'js/plugins/jquery.collapsibleTabs.js', 'version' => 6, 'class' => 'j.fn.collapsibleTabs' ),
				array( 'src' => 'js/plugins/jquery.color.js', 'version' => 1, 'class' => "j.fn.color" ),
				array( 'src' => 'js/plugins/jquery.cookie.js', 'version' => 4, 'class' => 'j.cookie' ),
				array( 'src' => 'js/plugins/jquery.delayedBind.js', 'version' => 1, 'class' => 'j.fn.delayedBind' ),
				array( 'src' => 'js/plugins/jquery.suggestions.js', 'version' => 14 , 'class' => 'j.fn.suggestions' ),
				array( 'src' => 'js/plugins/jquery.textSelection.js', 'version' => 33, 'class' => 'j.fn.textSelection' ),
				array( 'src' => 'js/plugins/jquery.wikiEditor.js', 'version' => 187 , 'class' => 'j.wikiEditor'),
				array( 'src' => 'js/plugins/jquery.wikiEditor.highlight.js', 'version' => 53, 'class' => 'j.wikiEditor.modules.highlight' ),
				array( 'src' => 'js/plugins/jquery.wikiEditor.toolbar.js', 'version' => 62 , 'class' => 'j.wikiEditor.modules.toolbar'),
				array( 'src' => 'js/plugins/jquery.wikiEditor.dialogs.js', 'version' => 24, 'class' => 'j.wikiEditor.modules.dialogs' ),
				array( 'src' => 'js/plugins/jquery.wikiEditor.toc.js', 'version' => 98 , 'class' => 'j.wikiEditor.modules.toc'),
				array( 'src' => 'js/plugins/jquery.wikiEditor.preview.js', 'version' => 11, 'class' => 'j.wikiEditor.modules.preview' ),
				array( 'src' => 'js/plugins/jquery.wikiEditor.templateEditor.js', 'version' => 77 , 'class' => 'j.wikiEditor.modules.templateEditor'),
				array( 'src' => 'js/plugins/jquery.wikiEditor.templates.js', 'version' => 1, 'class' => 'j.wikiEditor.modules.templates' ),
				array( 'src' => 'js/plugins/jquery.wikiEditor.publish.js', 'version' => 5 , 'class' => 'j.wikiEditor.modules.publish' ),
			),
			'combined' => array(
				array( 'src' => 'js/plugins.combined.js', 'version' => 396 ),
			),
			'minified' => array(
				array( 'src' => 'js/plugins.combined.min.js', 'version' => 396 ),
			),
		),
	);

	/* Static Functions */

	public static function initialize() {
		self::$doOutput = true;
	}

	/**
	 * AjaxAddScript hook
	 * Adds scripts
	 */
	public static function addResources( $out ) {
		global $wgExtensionAssetsPath, $wgJsMimeType;
		global $wgUsabilityInitiativeResourceMode;
		global $wgEnableJS2system, $wgEditToolbarRunTests;
		global $wgStyleVersion;

		wfRunHooks( 'UsabilityInitiativeLoadModules' );

		if ( !self::$doOutput )
			return true;

		// Default to raw
		$mode = $wgUsabilityInitiativeResourceMode; // Just an alias
		if ( !isset( self::$scriptFiles['base_sets'][$mode] ) ) {
			$mode = 'raw';
		}
		// Include base-set of scripts
		self::$scripts = array_merge(
			self::$scriptFiles['base_sets'][$mode],
			self::$scriptFiles['modules'][$mode],
			self::$scripts
		);
		// Provide enough support to make things work, even when js2 is not
		// in use (eventually it will be standard, but right now it's not)
		if ( !$wgEnableJS2system ) {
			$out->includeJQuery();
		}
		// Include base-set of styles
		self::$styles = array_merge(
			self::$styleFiles['base_sets'][$mode], self::$styles
		);
		if ( $wgEditToolbarRunTests ) {
			// Include client side tests
			self::$scripts = array_merge(
				self::$scripts, self::$scriptFiles['tests']
			);
		}
		// Loops over each script
		foreach ( self::$scripts as $script ) {
			// Add javascript to document
			if ( $script['src']{0} == '/' ) {
				// Path is relative to $wgScriptPath
				global $wgScriptPath;
				$src = "$wgScriptPath{$script['src']}";
			} else {
				// Path is relative to $wgExtensionAssetsPath
				$src = "$wgExtensionAssetsPath/UsabilityInitiative/{$script['src']}";
			}
			$version = isset( $script['version'] ) ? $script['version'] : $wgStyleVersion;
			if( isset( $script['class'] ) ){
				$out->addNamedResource( $script['class'] );
			} else {
				$out->addScriptFile( $src, $version );
			}
		}
		// Transforms messages into javascript object members
		foreach ( self::$messages as $i => $message ) {
			$escapedMessageValue = Xml::escapeJsString( wfMsg( $message ) );
			$escapedMessageKey = Xml::escapeJsString( $message );
			self::$messages[$i] =
				"'{$escapedMessageKey}':'{$escapedMessageValue}'";
		}
		// Add javascript to document
		if ( count( self::$messages ) > 0 ) {
			$out->addScript(
				Xml::tags(
					'script',
					array( 'type' => $wgJsMimeType ),
					'mw.usability.addMessages({' . implode( ',', self::$messages ) . '});'
				)
			);
		}
		// Loops over each style
		foreach ( self::$styles as $style ) {
			if( isset( $style['class'] ) ){
				$out->addStyleClass( $style['class'] );
			} else {
				// Add css for various styles
				$out->addLink(
					array(
						'rel' => 'stylesheet',
						'type' => 'text/css',
						'href' => $wgExtensionAssetsPath .
								"/UsabilityInitiativeJS2/" .
									"{$style['src']}?{$style['version']}",
					)
				);
			}
		}
		return true;
	}

	/**
	 * MakeGlobalVariablesScript hook
	 */
	public static function addJSVars( &$vars ) {
		$vars = array_merge( $vars, self::$variables );
		return true;
	}

	/**
	 * Adds a reference to a javascript file to the head of the document
	 * @param string $src Path to the file relative to this extension's folder
	 * @param object $version Version number of the file
	 */
	public static function addScript( $src, $version = '' ) {
		self::$scripts[] = array( 'src' => $src, 'version' => $version );
	}

	/**
	 * Adds a reference to a css file to the head of the document
	 * @param string $src Path to the file relative to this extension's folder
	 * @param string $version Version number of the file
	 */
	public static function addStyle( $src, $version = '' ) {
		self::$styles[] = array( 'src' => $src, 'version' => $version );
	}

	/**
	 * Adds internationalized message definitions to the document for access
	 * via javascript using the mw.usability.getMsg() function
	 * @param array $messages Key names of messages to load
	 */
	public static function addMessages( $messages ) {
		self::$messages = array_merge( self::$messages, $messages );
	}

	/**
	 * Adds variables that will be turned into global variables in JS
	 * @param $variables array of "name" => "value"
	 */
	public static function addVariables( $variables ) {
		self::$variables = array_merge( self::$variables, $variables );
	}

	/**
	 * Adds scripts for modules
	 * @param $scripts array with 'raw', 'combined' and 'minified' keys
	 */
	public static function addModuleScripts( $scripts ) {
		self::$scriptFiles['modules'] = array_merge(
			self::$scriptFiles['modules'], $scripts );
	}
}
