...
class MediaWikiFarmer
{
    ...
    public function __construct($params)
    {
        ...
        if (!is_dir($this->_configDirectory)) {
        	throw new Exception('configDirectory not found: '. $this->_configDirectory);
        } else {
    ...
    public function run()
    {
        ...
        if (!$this->_defaultWiki) {
            throw new Exception('Default wiki must be set');
        }
        ...
        if (is_callable($this->_matchFunction)) {
            ...
            throw new Exception('Function to map wiki name in farm not found: ' . print_r($this->_matchFunction, true));
        }
    }
    ...
    {   
        ...
        if (!$wiki->exists()) {
            ...
            if ($wiki->name == $this->_defaultWiki) {
                ...
                if (!$wiki->exists()) {
                    throw new Exception('MediaWikiFarmer could not write the default wiki configuration file.');
                } else {
                    $wiki->initialize();
                }
            } else {
                ...
                if (is_callable($this->_onUnknownWikiFunction)) {
                    ...
                    throw new Exception('Could not call function: ' . print_r($this->_onUnknownFunction, true));
                }
            }
        } else {
            //the wiki exists!
    ...
    protected function _populateMessagesForWiki($wiki, &$db = null)
    {
        ...
        if (is_callable($this->_defaultMessagesFunction)) {
            ...
            $defaultMessages = $this->getMWVariable('wgAllMessagesEn');          
        }
    ...
    protected function _writeExtensions()
    {
        ...
        if (file_put_contents($file, $content, LOCK_EX) != strlen($content)) {
            throw new Exception('Unable to write out extension file: ' . $file);
        }
    }
