...
class SpecialClickTracking extends SpecialPage {
	...
	function setDefaults() {
		$this->user_defs['basic'] = array(
			'anonymous' => '1',
			'total_contribs' => array( array( 'operation' => '<=', 'value' => '10' ) ),
		);
		...
		$this->user_defs['expert'] = array(
			'anonymous' => '0',
			'total_contribs' => array( array( 'operation' => '>=', 'value' => '400' ) ),
		);
	}
	...
	public function execute( $par ) {
		...
		$this->setHeaders();
		$this->setDefaults();
		$wgOut->addInlineScript( 'var wgClickTrackUserDefs = ' . json_encode( $this->user_defs ) );
	 ...
	 *
	 * @param $event_id Integer: event ID this chart is for
	 * @param $minTime Integer: minimum day
	 * @param $maxTime Integer: maximum day
	 * @param $increment Integer: number of day(s) to increment in units
	 * @param $userDefs Array: user defintions
	 * @param $isUserDefsJSON Boolean: true if userDefs is JSON
	...
	static function getChartData( $event_id, $minTime, $maxTime, $increment, $userDefs, $isUserDefsJSON = true ) {
		...
		return array(
			'expert' => $expertUserData,
			'basic' => $basicUserData,
			'intermediate' => $intermediateUserData
		);
	}

	function buildChart( $event_name, $event_id, $minTime, $maxTime, $increment ) {
		$chartData = self::getChartData(
			$event_id, $minTime, $maxTime,
			$increment, $this->user_defs, false
		);
		$chartSrc = $this->getGoogleChartParams(
			$event_id,
			$event_name,
			$minTime,
			$maxTime,
			$chartData['basic'],
			$chartData['intermediate'],
			$chartData['expert']
		);
	...
	function getGoogleChartParams(
		$event_id, $event_name, $minDate, $maxDate, $basicUserData, $intermediateUserData, $expertUserData
	) {
	...
	function buildUserDefBlankDialog() {
		...
		$control .= Xml::openElement(
			'input', array( 'type' => 'checkbox', 'id' => 'anon_users_checkbox', 'class' => 'user_def_checkbox' )
		);
		...
		$control .= Xml::openElement(
			'div', array( 'id' => 'total_users_contrib_div', 'class' => 'checkbox_div control_div' )
		);
		$control .= Xml::openElement(
			'input', array( 'type' => 'checkbox', 'id' => 'contrib_checkbox', 'class' => 'user_def_checkbox' )
		);
		$control .= Xml::closeElement( 'input' );
		...
		$control .= Xml::openElement(
			'div', array( 'id' => 'contrib_span_1_div', 'class' => 'checkbox_div control_div' )
		);

		$control .= Xml::openElement( 'div', array( 'id' => 'contrib_span_1_text_div', 'class' => 'checkbox_div' ) );
		$control .= Xml::openElement(
			'input', array( 'type' => 'checkbox', 'id' => 'contrib_span_1_checkbox', 'class' => 'user_def_checkbox' )
		);
		$control .= Xml::closeElement( 'input' );
		...
		$control .= Xml::openElement(
			'div', array( 'id' => 'contrib_span_2_div', 'class' => 'checkbox_div control_div' )
		);

		$control .= Xml::openElement( 'div', array( 'id' => 'contrib_span_2_text_div', 'class' => 'checkbox_div' ) );
		$control .= Xml::openElement(
			'input', array( 'type' => 'checkbox', 'id' => 'contrib_span_2_checkbox', 'class' => 'user_def_checkbox' )
		);
		$control .= Xml::closeElement( 'input' );
		...
		$control .= Xml::openElement(
			'div', array( 'id' => 'contrib_span_3_div', 'class' => 'checkbox_div control_div' )
		);

		$control .= Xml::openElement( 'div', array( 'id' => 'contrib_span_3_text_div', 'class' => 'checkbox_div' ) );
		$control .= Xml::openElement(
			'input', array( 'type' => 'checkbox', 'id' => 'contrib_span_3_checkbox', 'class' => 'user_def_checkbox' )
		);
		$control .= Xml::closeElement( 'input' );
	...
	function buildUserDefNumberSelect( $include_checkbox, $include_and, $ids ) {
		...
		if ( $include_checkbox ) {
			$control .= Xml::openElement(
				'input', array( 'type' => 'checkbox', 'id' => "{$ids}_checkbox", 'class' => 'number_select_checkbox' )
			);
			$control .= Xml::closeElement( 'input' );
		}
		...
		$control .= Xml::openElement(
			'option', array( 'id' => '{$ids}_lt', 'class' => 'number_select_ltgt_opt', 'value' => 'lt' )
		);
		$control .= '&lt;=';
		$control .= Xml::closeElement( 'option' );
		$control .= Xml::openElement(
			'option', array( 'id' => '{$ids}_gt', 'class' => 'number_select_ltgt_opt', 'value' => 'gt' )
		);
		$control .= '&gt;=';
		$control .= Xml::closeElement( 'option' );
		$control .= Xml::closeElement( 'select' );
		$control .= Xml::openElement(
			'input', array( 'type' => 'text', 'id' => "{$ids}_text", 'class' => 'number_select_text' )
		);
		$control .= Xml::closeElement( 'input' );
		return $control;
	}
	...
	function buildChartDialog() {
		...
		$control .= Xml::openElement(
			'input',
			array( 'type' => 'text', 'id' => 'chart_increment', 'class' => 'chart_dialog_area', 'value' => '1' )
		);
		$control .= Xml::closeElement( 'input' );
		...
		$control .= Xml::openElement(
			'input', array( 'type' => 'button', 'id' => 'change_graph', 'value' => wfMsg( 'ct-change-graph' ) )
		);
		$control .= Xml::closeElement( 'input' );
		$control .= Xml::closeElement( 'td' );
	...
	function buildDateRange() {
		...
		$control .= Xml::openElement(
			'input',
			array(
				'type' => 'checkbox', 'id' => 'start_date_checkbox', 'class' => 'date_range_checkbox', 'checked' => ''
			)
		);
		$control .= Xml::closeElement( 'input' );
		...
		$control .= Xml::openElement(
			'input', array( 'type' => 'text', 'id' => 'start_date', 'class' => 'date_range_input' )
		);
		$control .= Xml::closeElement( 'input' );
		$control .= Xml::closeElement( 'td' );
		...
		$control .= Xml::openElement(
			'input',
			array(
				'type' => 'checkbox', 'id' => 'end_date_checkbox', 'class' => 'date_range_checkbox', 'checked' => ''
			)
		);
		$control .= Xml::closeElement( 'input' );
		...
		$control .= Xml::openElement(
			'input', array( 'type' => 'text', 'id' => 'end_date', 'class' => 'date_range_input' )
		);
		$control .= Xml::closeElement( 'input' );
		$control .= Xml::closeElement( 'td' );
	...
	function buildRow( $data_result, $row_count, $userDefs ) {
		...
		$outputRow .= Xml::openElement( 'td',
			array(
				'class' => 'event_data expert_data',
				'id' => "event_expert_$row_count",
				'value' => $cellValue
			)
		);
		$outputRow .= $cellValue;
		...
		$outputRow .= Xml::openElement( 'td',
			array(
				'class' => 'event_data intermediate_data',
				'id' => "event_intermediate_$row_count",
				'value' => $cellValue
			)
		);
		$outputRow .= $cellValue;
		...
		$outputRow .= Xml::openElement( 'td',
			array(
				'class' => 'event_data basic_data',
				'id' => "event_basic_$row_count",
				'value' => $cellValue
			)
		);
		$outputRow .= $cellValue;
	 ...
	 *
	 * @param $dateWithoutSpaces Integer: date without spaces
	 * @return date with spaces
	 */
	public static function spaceOutDate( $dateWithoutSpaces ) {
		return (
			substr( $dateWithoutSpaces, 0, 4 ) . ' ' .
			substr( $dateWithoutSpaces, 4, 2 ) . ' ' .
			substr( $dateWithoutSpaces, 6, 2 )
		);
	}
	 ...
	 *
	 * @param $minTime Integer: minimum day (YYYYMMDD)
	 * @param $maxTime Integer: max day (YYYYMMDD)
	 * @return Array
	 * NOTE: once some of the constraints have been finalized, this will use more of the Database functions and not raw
	 * SQL
	 */
	static function getTimeConstraints( $minTime, $maxTime ) {
		if (
			$minTime == 0 || $maxTime == 0 ||
			( strptime( SpecialClickTracking::spaceOutDate( $minTime ), "%Y %m %d" ) === false ) ||
			( strptime( SpecialClickTracking::spaceOutDate( $minTime ), "%Y %m %d" ) === false )
		)
		{
			return array();
	...
	public static function getTopEvents( $minTime = '', $maxTime = '', $normalize_top_results = false ) {
		$timeConstraintStatement = self::getTimeConstraints( $minTime, $maxTime );
		$timeConstraint = $timeConstraintStatement;
	 ...
	 *
	 * @param $def Array
	 * @return Array: array containing the built SQL query
	 */
	public static function buildUserDefConstraints( $def ) {
		...
		if ( !empty( $total_contribs ) ) {
			foreach ( $total_contribs as $contribs ) {
				$and_conds[] = array(
					'field' => 'user_total_contribs',
					'operation' => SpecialClickTracking::validateOperation( $contribs['operation'] ),
					'value' => intval( $contribs['value'] )
				);
			}
		}
