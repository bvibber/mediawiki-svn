...
class FlagArticle extends SpecialPage {
	function __construct() {
		parent::__construct( 'FlagArticle' );
	 ...
	 * 
	 * @param $par String: pagetitle in Special:FlagArticle/pagetitle
	 */
	function execute( $par ) {
		global $wgRequest, $wgOut, $wgTitle;
		$this->setHeaders();
		$page = $wgRequest->getText( 'page' );
		# $page is set via subtitle (Special:FlagArticle/pagetitle)
		if ( isset( $par ) )
			$page = $par;
		...
		{
			$wgOut->showErrorPage('flagarticle-nopageselectedtitle','flagarticle-nopageselected');
		}
	}
	...
	function selection( $page ) {
		global $wgOut;
		$templatelist = new Article( Title::newFromText( 'flagarticle-templatelist', NS_MEDIAWIKI ) );
		$templatelistcontent = $templatelist->fetchContent();
		if ( $templatelistcontent=="" ) # empty config list
		{
			$wgOut->showErrorPage( 'flagarticle-emptylisttitle','flagarticle-emptylist' );
		}
	...
	function replaceLinks( $page, $wikitext ) {
		...
		$wikitext = str_replace("[[", "[{{fullurl:Special:FlagArticle|page=" . $page . "&template=" , $wikitext);		// FIXME: something like wfMsgForContent() for aliases is needed to use the local name of Special:FlagArticle
		$wikitext = str_replace("]]", "]", $wikitext);
		return $wikitext;
	}
	...
	function preview( $page, $template ) {
		...
		global $wgOut, $wgUser;
		$wgOut->addWikiText( wfMsg( 'flagarticle-preview' ) );
		$wgOut->addWikiText('<div class="flagarticle-preview" style="background-color:#F5F5F5; border:1px solid #AAAAAA; padding:0 0.8em 0.3em 0.5em;"> {{' . $template . "}}</div>");
		$token = $wgUser->editToken();
		// TODO what a mess! tidy up the html code
		$s = Xml::openElement( 'form', array( 'method' => 'post', 'action' => $this->getTitle()->getLocalURL(), 'id' => 'mw-flagarticle-form' ) ) .
		Xml::openElement( 'p' ) .
		Xml::tags( 'label', null, wfMsg( 'flagarticle-confirmsave' ) ) .
		Xml::hidden( 'page', $page ) .
		Xml::hidden( 'template', $template ) .
		Xml::hidden( 'token', $token ) .
		Xml::submitButton( wfMsg( 'flagarticle-submitbutton' ) ) ."\n" .
		Xml::closeElement( 'p' ) . "\n" .
	...
	function submit( $page, $template, $token ) {
		...
		if ($id==0) { # Article does not exist. Show preview page
			$wgOut->addWikiMsg( 'flagarticle-nonexistent', $page );
			$this->preview( $page, $template );
			return;
		}
		$article = Article::newFromId( $id );
		$text = "{{" . $template . "}}\n\n" . $article->getRawText();
		$summary = wfMsg( 'flagarticle-summary', $template );
		$article->doEdit( $text, $summary, EDIT_UPDATE, $id );
		$wgOut->addWikiMsg( 'flagarticle-success', $template, $page, $id ); // FIXME Broken link without oldid
