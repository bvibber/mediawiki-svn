...
$(this).data('suggestions-context',context);});return returnValue!==null?returnValue:$(this);};})(jQuery);(function($){$.fn.textSelection=function(command,options){var fn={getContents:function(){return $(this).val();},getSelection:function(){var retval='';if($(this).is(':hidden')){}else if(document.selection&&document.selection.createRange){this.focus();var range=document.selection.createRange();retval=range.text;}else if(this.selectionStart||this.selectionStart=='0'){retval=this.value.substring(this.selectionStart,this.selectionEnd);}
...
context.fn={'getContents':function(){return $('<div />').html(context.$content.html().replace(/\<br\>/g,"\n")).text();},'getSelection':function(){},'encapsulateSelection':function(options){},'getCaretPosition':function(options){},'setSelection':function(options){},'scrollToCaretPosition':function(options){}};}
...
return sections;}
function buildList(structure){var list=$('<ul></ul>');for(i in structure){var div=$('<div></div>').attr('href','#').addClass('section-'+structure[i].index).data('textbox',context.$textarea).data('position',structure[i].position).bind('mousedown',function(event){$(this).data('textbox').focus().textSelection('setSelection',{'start':$(this).data('position')}).textSelection('scrollToCaretPosition',{'force':true});if(typeof $.trackAction!='undefined')
