...
mvRemoteSearch.prototype = {
	...
	init:function( initObj ){		
		...
		var box_dark = (this.result_display_mode=='box')?'_dark':'';
		var list_dark = (this.result_display_mode=='list')?'_dark':'';
		$j('#'+this.target_submit).after('<img id="msc_box_layout" ' +
				'src = "' + mv_embed_path + 'skins/' + mv_skin_name + '/images/box_layout_icon' + box_dark + '.png" ' +			
				'style="width:20px;height:20px;cursor:pointer;"> ' + 
			'<img id="msc_list_layout" '+
				'src = "' + mv_embed_path + 'skins/' + mv_skin_name + '/images/list_layout_icon' + list_dark + '.png" '+			
				'style="width:20px;height:20px;cursor:pointer;">'
		);
		js_log('should have set: ' +this.target_submit);		
		$j('#msc_box_layout').hover(function(){			
			$j(this).attr("src", mv_embed_path+'skins/'+mv_skin_name+'/images/box_layout_icon_dark.png');
		}, function(){
			var box_dark = (_this.result_display_mode=='box')?'_dark':'';
			$j(this).attr("src", mv_embed_path+'skins/'+mv_skin_name+'/images/box_layout_icon'+box_dark+'.png');		
		}).click(function(){	
			$j(this).attr("src", mv_embed_path+'skins/'+mv_skin_name+'/images/box_layout_icon_dark.png');		
		...
		$j('#msc_list_layout').hover(function(){
			$j(this).attr("src", mv_embed_path+'skins/'+mv_skin_name+'/images/list_layout_icon_dark.png');
		}, function(){
			var list_dark = (_this.result_display_mode=='list')?'_dark':'';
			$j(this).attr("src", mv_embed_path+'skins/'+mv_skin_name+'/images/list_layout_icon'+list_dark+'.png');		
		}).click(function(){
			$j(this).attr("src", mv_embed_path+'skins/'+mv_skin_name+'/images/list_layout_icon_dark.png');
