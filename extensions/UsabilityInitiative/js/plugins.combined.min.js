...
var oldSC=sel.getRangeAt(0).startContainer;var oldSO=sel.getRangeAt(0).startOffset;sel.extend(sc,options.start);if(oldSC==sel.getRangeAt(0).startContainer&&oldSO==sel.getRangeAt(0).startOffset){sel.collapseToEnd();}else{sel.collapseToStart();}
if(options.end!=options.start||sc!=ec){sel.extend(ec,options.end);}
context.$iframe[0].contentWindow.focus();},'scrollToCaretPosition':function(options){},'scrollToTop':function($element,force){var html=context.$content.closest('html'),body=context.$content.closest('body');var y=$element.offset().top-context.$content.offset().top;if(force||y<html.scrollTop()||y<body.scrollTop()||y>html.scrollTop()+context.$iframe.height()||y>body.scrollTop()+context.$iframe.height()){html.scrollTop(y);body.scrollTop(y);}
...
tokenArray.push(new Token(match.index+oldOffset+markOffset,label,tokenStart));oldOffset+=match.index+match[0].length;newSubstring=text.substring(oldOffset);match=newSubstring.match(regex);}}}}
