...
( function( $ ) {
	$.fn.sequencer = function( options, callback ) {
		...
		if ( !mw['mvSeq'] ) {
			mw['mvSeq'] = new mw.SequenceEdit( options );
		} else {
...
mw.SequenceEdit.prototype = {
	...
	init: function( iObj ) {
		...
		if ( this.smilSource != 'null' ) {
			mw.log( 'smilSource:: ' + this.smilSource );
			var src_attr = ' src="' + this.smilSource + '" ';
		} else {
			mw.log( ' null playlist src .. (start empty) ' );
			var src_attr = '';
		}
		$j( '#' + this.video_container_id ).append(
			$j('<video />').css({
				'width': this.video_width,
				'height': this.video_height
			}).attr({
				'id' : this.smilPlayerId				
			}).append(
				$j('<source />').attr({
		...
		$j('#' + this.smilPlayerId ).embedPlayer();
	...
	checkReadyPlObj: function() {
		...
		this.plObj = $j( '#' + this.smilPlayerId ).get( 0 );
	...
	plReadyInit:function() {
		...
		if ( this.plObj.getClipCount() == 0 ) {
			$j( '#' + this.smilPlayerId ).html( 'empty playlist' );
		}
