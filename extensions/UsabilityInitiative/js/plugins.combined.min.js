...
var visited=[],v=0;var purgeStarts=false;var cBreak=false;for(var i=0;i<markers.length;i++){if(!purgeStarts&&((markers[i].start+","+markers[i].type)in context.modules.highlight.markersOldStarts)){oldStarts[markers[i].start+","+markers[i].type]=true;if(division=='realchange'){cBreak=true;}}
else{if(!purgeStarts){context.modules.highlight.markersOldStarts=oldStarts;purgeStarts=true;if(division=='realchange'){cBreak=true;}}
...
ranges[i].end+=adjustment;}};if(wikitext.length<20){collapsible=false;}
if(wikitext.match(/\s*{{\s*\S*:/)){collapsible=false;}
