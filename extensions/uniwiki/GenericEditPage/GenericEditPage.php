...
$wgExtensionCredits['other'][] = array(
	'name'        => "Uniwiki Generic Edit Page",
...
$wgCategoryPage             = "MediaWiki:Editpagetags";
...
require_once ("GenericEditPage.i18n.php");
$wgExtensionFunctions[] = "UW_GenericEditPage_i18n";

function UW_GenericEditPage_i18n() {
	// add this extension's messages to the message cache
	global $wgMessageCache, $wgGenericEditPageMessages;
	foreach ($wgGenericEditPageMessages as $lang => $messages)
...
function UW_GenericEditPage_emailSuggestion ($category) {
	...
	return WikiError::isError ($result)
	     ? wfMsg ('gep_emailfailure')
	     : wfMsg ('gep_emailsuccess', $category);
}
...
function UW_GenericEditPage_extractCategoriesIntoBox(&$text) {
	...
	if ($wgAddCategory) {
		$out .= "
			<div class='add'>
				<label for='fm-add-cat'>".wfMsg ('gep_addcategory')."</label>
				<input type='text' id='fm-add-cat' autocomplete='off' />
				<input type='button' value='".wfMsg ('gep_addcategorybutton')."' id='fm-add-cat-button' />
			</div>
	...
	if ($wgSuggestCategory) {
		$out .= "
			<div class='suggest'>
				<label for='fm-suggest-cat'>".wfMsg ('gep_suggestcategory')."</label>
				<input type='text' id='fm-suggest-cat' autocomplete='off' />
				<input type='button' value='".wfMsg ('gep_suggestcategorybutton')."' id='fm-suggest-cat-button' />
			</div>
			<script type='text/javascript'>
...
function UW_GenericEditPage_renderSectionBox ($sections) {
		...
		if ($sections[$i]['required']) {
			...
			$out .= "
				<div id='sect-$i' class='section-box disabled'>
					...
					<label title='".wfMsg ('gep_sectionnotdisabled')."'>{$sections[$i]['title']}</label>
				</div>
			";
		} else {
	...
	if ($wgAddSection) {
		$out .= "
			<div class='add'>
				<label for='fm-add-sect'>".wfMsg ('gep_addsection')."</label>
				<input type='text' id='fm-add-sect' autocomplete='off' />
				<input type='button' value='".wfMsg ('gep_addsectionbutton')."' id='fm-add-sect-button' />
			</div>
			<script type='text/javascript'>
...
function UW_GenericEditPage_displayEditPage ($editor, $out) {
	...
	$out->addHTML("
		...
		<div class='generic-editor $gen_editor_class'>
			<div class='instructions'>".wfMsg ('gep_nosectioninstructions')."</div>
	");
