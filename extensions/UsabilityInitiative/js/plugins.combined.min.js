...
conf.suggestions=[];conf._data={};conf._data.textbox=$(this);conf._data.timerID=null;conf._data.prevText=null;conf._data.visibleResults=0;conf._data.mouseDownOn=$([]);conf._data.div=$('<div />').addClass('os-suggest').css({top:Math.round($(this).offset().top)+this.offsetHeight,left:Math.round($(this).offset().left),width:$(this).outerWidth()}).hide().appendTo($('body'));$('<table />').addClass('os-suggest-results').width($(this).outerWidth()).appendTo(conf._data.div);$(this).attr('autocomplete','off').keydown(function(e){conf._data.keypressed=(e.keyCode==undefined)?e.which:e.keyCode;conf._data.keypressed_count=0;}).keypress(function(){if(conf._data.keypressed==38||conf._data.keypressed==40){var now=new Date().getTime();if(now-conf._data.last_keypress<120){return;}}
conf._data.last_keypress=now;conf._data.keypressed_count++;processKey(conf._data.keypressed);}).keyup(function(){conf._data.last_keypress=0;if(conf._data.keypressed_count==0)
...
$(this).trigger('scrollToPosition');});}});})(jQuery);jQuery.wikiEditor={'modules':{},'instances':[]};(function($){$.fn.wikiEditor=function(){var context=$(this).data('context');if(typeof context!=='undefined'){arguments=$.makeArray(arguments);if(arguments.length>0){var call=arguments.shift();if(call in context.api){context.api[call](context,arguments[0]==undefined?{}:arguments[0]);}
