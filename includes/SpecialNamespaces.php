...
function showForm( $errorHeader='', $errorBody='' ) {
	...
	$name_array = $nsstore->getFormattedDefaultNamespaces(true);
...
END;
		...
		$nsobjects = $nsstore->getAllNamespaceObjects();
...
END;
				$target_html = $this->selectorRow( 'namespace_default_link_prefix',
					"ns{$index}Linkprefix",
					$nsstore->getIndexForName( $ns->getTarget() ) );
				$special_html='';
...
END;
	...
	$all_name_array = $nsstore->getFormattedDefaultNamespaces(true);
...
<input type="submit" value="'.wfMsgHtml('pseudonamespace_convert').'" />
	...
	*/
	...
	function deleteNamespace() {
	    ...
	    $ns = $nsstore->getNamespaceObjectByIndex($nsid);
	...
	function fixPseudonamespaces() {
		...
		$mainnsobj = $nsstore->getNamespaceObjectByIndex(NS_MAIN);
		$maintalknsobj = $nsstore->getNamespaceObjectByIndex(NS_TALK);
		$rv=$nsstore->convertPseudonamespace($prefix,$nsobj,$mainnsobj,$merge);
		if($rv[NS_RESULT]!=NS_PSEUDO_CONVERTED) {
			...
			$wgOut->addWikiText(wfMsg('pseudonamespace_converted', $prefix, $nsobj->getDefaultName()));
			$this->logNs('pseudo',$prefix,$nsobj->getDefaultName());
		}
		if($converttalk) {
			# A pseudonamespace, by definition, exists in the 
			# main (unprefixed) namespace - therefore its talk
			# pages are NS_TALK.
			$trv=$nsstore->convertPseudonamespace($prefix,$talk4nsobj,$maintalknsobj,$merge);
	...
	function selectorRow( $labelMessage, $name, $selected ) {
		...
		return '<tr><td colspan="2">' .
			...
			"<div>" .
				...
				$this->getSelector( $nsstore->getFormattedDefaultNamespaces(true), $selected ) .
