	...
	*************************/
	...
	function doInlineEditForm() {
		...
		if ( $method == 'reply' ) {
			...
			wfRunHooks( 'LiquidThreadsDoInlineEditForm', array( $thread,$this->request, &$this->output ) );
		}
	...
	function showNewThreadForm( $talkpage ) {
		...
		wfRunHooks( 'LiquidThreadsEndOfShowNewThreadForm', array( &$e, $talkpage ) );
	...
	function showPostEditingForm( $thread ) {
        ...
        $article = $thread->root();
		$talkpage = $thread->article();
        wfRunHooks( 'LiquidThreadsEditingFormContent', array( $thread, &$article , $talkpage) );
