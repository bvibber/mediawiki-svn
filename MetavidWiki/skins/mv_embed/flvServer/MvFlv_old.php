...
define('META_DATA_EXT', '.KfMeta'); //file extension for cached Time metadata 

 class MvFLV extends FLV {
	...
	private $kfMeta=array();
	
	private $metaDuration=0;
	...
	function playTimeReq($start_time_sec, $end_time_sec=null){	
		...
		if( $end_time_sec)
			$this->metaDuration = $end_time_sec - $start_time_sec;
		//print "SET metaDuration to:  " . $this->metaDuration . "\n";		
		$meta = & $this->getKfMetaData();
		...
		for($i=0;$i<count($meta['times']);$i++){
			...
			if($end_time_ms!=null){
				if($meta['times'][$i]>=$end_time_ms)$end_byte= $meta['filepositions'][$i];
			}		
		}
	...
	function getKfMetaData()
	{		
		if(is_file($this->fname . META_DATA_EXT)){
			$this->kfMeta = unserialize(file_get_contents ($this->fname . META_DATA_EXT));			
		}else{
			$this->computeMetaData();
			//store keframe data as int (takes less time to parse/open/search) 
			foreach($this->compMetaData['keyframes']['times'] as $inx=>&$ts){
				$ts = (int)($ts*1000);
			} 
			if(!file_put_contents($this->fname . META_DATA_EXT, serialize($this->compMetaData['keyframes']))){
	...
	{		
		...
		//ignore the "ture" metadata and make a simple metadata packet:		
		if (! empty($meta))
		{
