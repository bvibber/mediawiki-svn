...
p=p.parentNode;var prev=p.previousSibling;p=this;while(p&&!p.nextSibling)
p=p.parentNode;var next=p.nextSibling;if(prev&&prev.nodeName=='#text'){var prevText=prev.nodeValue;while(prevText.substr(-1)=='='){prevText=prevText.substr(0,prevText.length-1);text='='+text;}
...
if('periRegex'in action.options&&'periRegexReplace'in action.options){<<<<<<<.mine
var selection=context.$textarea.textSelection();=======var selection=context.$textarea.textSelection('getSelection');>>>>>>>.r58906
if(selection!=''){parts.peri=selection.replace(action.options.periRegex,action.options.periRegexReplace);}}
