...
public class WikiConfiguration_nlwiki extends WikiConfiguration {
	...
	public WikiConfiguration_nlwiki() {
		...
		conceptTypeSensors.add( new HasCategoryLikeSensor(ConceptType.PERSON, "(^|_)persoon(_|$)|(.*schapper|.*oloog|.*icus)$", 0));
		conceptTypeSensors.add( new HasTemplateLikeSensor(ConceptType.PERSON, "^(Infobox_(artiest|Auteur|acteur|Comedian|.*speler|Presentator|regisseur)|Winnaars_.*)$", Pattern.CASE_INSENSITIVE));
		conceptTypeSensors.add( new HasTemplateLikeSensor(ConceptType.PERSON, "^(Infobox_.*|.*cus|.*eur|.*ler|.*schapper)$", Pattern.CASE_INSENSITIVE, new String[] {"geboren"}));
		conceptTypeSensors.add( new HasSectionLikeSensor(ConceptType.PERSON, "^((.* )?leven|Carri\u00e8re|Stamvader|Levensloop|Filmografie|Bibliografie|publicaties|(Eigen )?Biografie|Priv\u00e9|.*Loopbaan.*|Jeugd|Kinderen|Familie|Familieachtergrond)$", Pattern.CASE_INSENSITIVE) );

		//conceptTypeSensors.add( new WikiTextAnalyzer.TitleSensor(".*_\\(voornaam\\)", 0));
		conceptTypeSensors.add( new HasCategorySensor(ConceptType.NAME, "Jongensnaam"));
