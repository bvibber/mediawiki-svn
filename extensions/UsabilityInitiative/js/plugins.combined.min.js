...
if(lastNode)
context.fn.scrollToTop(lastNode);return context.$textarea;},'getCaretPosition':function(options){},'setSelection':function(options){},'scrollToCaretPosition':function(options){},'scrollToTop':function($element,force){var body=context.$content.closest('body');var y=$element.offset().top-context.$content.offset().top;if(force||y<body.scrollTop()||y>body.scrollTop()+body.height())
body.scrollTop(y);}};}
...
context.modules.$toc.scrollTop(scrollTop+relTop+sectionHeight-divHeight);}},collapse:function(event){var context=event.data;var $toc=context.modules.$toc;if(!$toc.data('openWidth')){$toc.data('openWidth',$toc.width());}
context.$ui.find('.tab-toc').unbind('click',$.wikiEditor.modules.toc.fn.collapse).bind('click',context,$.wikiEditor.modules.toc.fn.expand).children('a').text(gM('wikieditor-toc-show'));$toc.animate({'width':1},'fast');$.cookie('wikiEditor-'+context.instance+'-toc-width',1);return false;},expand:function(event){var context=event.data;context.$ui.find('.tab-toc').unbind('click',$.wikiEditor.modules.toc.fn.expand).bind('click',context,$.wikiEditor.modules.toc.fn.collapse).children('a').text(gM('wikieditor-toc-hide'));context.modules.$toc.show().animate({'width':context.modules.$toc.data('openWidth')},'fast');$.cookie('wikiEditor-'+context.instance+'-toc-width',context.modules.$toc.data('openWidth'));return false;},drag:function(e,pageX){var context=e.data;var mR;if(pageX)
mR=pageX;else
mR=e.pageX-context.$ui.find('.wikiEditor.ui-left').offset().left;mR=context.$ui.find('.wikiEditor-ui-left').width()-mR;if(mR<26||mR>context.$ui.find('.wikiEditor-ui-left').width()-250)
return false;context.$ui.find('.wikiEditor-ui-left').css('marginRight',-mR).children().css('marginRight',mR);context.$ui.find('.wikiEditor-ui-right').css('width',mR);return false;},stopDrag:function(e){var context=e.data;$().unbind('mousemove',$.wikiEditor.modules.toc.fn.drag).unbind('mouseup',$.wikiEditor.modules.toc.fn.stopDrag);$(context.$iframe[0].contentWindow.document).unbind('mousemove').unbind('mouseup');if(context.$ui.find('.wikiEditor-ui-right').width()<50&&wgNavigableTOCCollapseEnable){$.wikiEditor.modules.toc.fn.collapse({data:context});}else{context.$ui.find('.wikiEditor-ui-left').trigger('mouseup');context.$ui.find('.wikiEditor-ui-right').data('openWidth',context.$ui.find('.wikiEditor-ui-right').width());$.cookie('wikiEditor-'+context.instance+'-toc-width',context.$ui.find('.wikiEditor-ui-right').width());}
return false;},build:function(context){function buildStructure(outline,offset,level){if(offset==undefined)offset=0;if(level==undefined)level=1;var sections=[];for(var i=offset;i<outline.length;i++){if(outline[i].nLevel==level){var sub=buildStructure(outline,i+1,level+1);if(sub.length){outline[i].sections=sub;}
...
$collapseControl.insertBefore(context.modules.$toc);if(!context.modules.$toc.data('openWidth')){context.modules.$toc.data('openWidth',context.initialWidth==1?$.wikiEditor.modules.toc.defaultWidth:context.initialWidth);}
if(context.initialWidth==1)
$.wikiEditor.modules.toc.fn.collapse({data:context});return $resizeControlVertical;}
