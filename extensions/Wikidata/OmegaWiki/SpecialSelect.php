...
function getSelectOptions() {
	...
	$sql = 'SELECT language_id' .
			" FROM {$dc}_syntrans" .
			" JOIN {$dc}_expression_ns ON {$dc}_expression_ns.expression_id = {$dc}_syntrans.expression_id" .
	...
	while ($options_row = $dbr->fetchObject($options_res)) {
		...
		if ($lang_code == 'en' || !($lang_id = getLanguageIdForCode($lang_code))) {
			$sql = "SELECT {$dc}_expression_ns.spelling" .
					" FROM {$dc}_syntrans" .
					" JOIN {$dc}_expression_ns ON {$dc}_expression_ns.expression_id = {$dc}_syntrans.expression_id" .
					" WHERE {$dc}_syntrans.defined_meaning_id = " . $options_row->option_mid .
					" AND {$dc}_expression_ns.language_id = " . getLanguageIdForCode('en') .
					' AND ' . getLatestTransactionRestriction("{$dc}_syntrans") .
					' AND ' . getLatestTransactionRestriction("{$dc}_expression_ns");
		}
		/* Fall back on English in cases where an option name is not present in the
			...
			$sql = "SELECT {$dc}_expression_ns.spelling" .
					...
					" JOIN {$dc}_expression_ns ON {$dc}_expression_ns.expression_id = {$dc}_syntrans.expression_id" .
					" WHERE {$dc}_syntrans.defined_meaning_id = " . $options_row->option_mid .
					" AND {$dc}_expression_ns.language_id = " . $lang_id .
					' AND ' . getLatestTransactionRestriction("{$dc}_syntrans") .
					' AND ' . getLatestTransactionRestriction("{$dc}_expression_ns");
			$res = $dbr->query($sql);
