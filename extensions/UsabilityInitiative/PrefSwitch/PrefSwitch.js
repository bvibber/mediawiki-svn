...
$j(document).ready( function() {
	$j( '.prefswitch-other-select' ).parent().hide();
	$j( 'select.prefswitch-need-other' ).change( function() {
		if( $j(this).val() == 'other' )
			$j( '#prefswitch-' + $j(this).attr( 'id' ) + '-other' ).parent().slideDown( 'fast' );
		else
			$j( '#prefswitch-' + $j(this).attr( 'id' ) + '-other' ).parent().slideUp( 'fast' );
	});
	$j( '.prefswitch-other-radios, .prefswitch-other-checks' ).click( function() {
		$j(this).prev().prev().attr( 'checked', true );
	});
	$j( '.survey-ifyes, .survey-ifno' ).hide();
	$j( '.survey-yes, .survey-no' ).change( function() {
		yesrow = $j( '#prefswitch-' + $j(this).attr( 'name' ) + '-ifyes-row' );
		norow = $j( '#prefswitch-' + $j(this).attr( 'name' ) + '-ifno-row' );
	...
	if ( detected['survey-res-x'] && detected['survey-res-y'] ) {
		$j( '.prefswitch-survey-resolution-x' ).val( detected['survey-res-x'] );
		$j( '.prefswitch-survey-resolution-y' ).val( detected['survey-res-y'] );
	}
});
