...
class DPLForum {
	...
	function get($name, $value=NULL, $parser=NULL) {
		if(preg_match("/^\s*$name\s*=\s*(.*)/mi",$this->sInput,$matches)) {
			...
			else return $parser->replaceVariables($arg);
		}
		return $value;
	}
	...
	function link(&$parser, $count, $page='', $text='') {
		...
		$i = intval($page);
		if(($i!=0) && ctype_digit($page[0])) $i -= 1;
		else $i += intval($offset / $count);
		if($this->link_test($i,$page)) return '';
	...
	function parse(&$input, &$parser) {
		...
		else $this->bShowNamespace = ($this->get('shownamespace')=='true');
		...
		($total < $this->minCategories))) return $this->msg('dplforum-toofew',1);
	...
	function buildOutput($page, $title, $time, $user='', $author='', $made='') {
		...
		if($this->bAddCreationDate) {
			...
			if($tm) $output .= "<td class='forum_created'>$made</td>";
			else if($made) $output = "{$made}: ";
		}
		if($tm) $output .= "<td class='forum_title'>";

		$text = $query = $props = '';
		if($this->bShowNamespace == true) $text = $title->getEscapedText();
		...
		if($this->bAddAuthor) {
			...
			if($tm) {
				...
				else $output .= "</td><td class='forum_author'>$author";
			}
		...
		if($this->bAddLastEdit) {
			...
			if($tm) $output .= "</td><td class='forum_edited'>$time";
			else $text .= "$time ";
		}
		...
		if($this->bAddLastEditor) {
			...
			if($tm) {
				...
				else $output .= "</td><td class='forum_editor'>$user";
			}
			else if($user) $text .= "$by $user";
		}
		if($tm) $output .= "</td>";
		else if($text) $output .= $this->msg('dplforum-edited') . " $text";
		return $output;
	}
}
