...
final class ImageListUI(programHeading:String, programIcon:Image) extends JPanel {
	...
	add(selectLabel,		GBC.pos(0,0).size(1,1).weight(0.001,0).anchorCenter().fillHorizontal().insetsAll(0,0,0,2))
	add(selectNoneButton,	GBC.pos(1,0).size(1,1).weight(0.001,0).anchorCenter().fillHorizontal().insetsAll(0,0,0,2))
	add(selectAllButton,	GBC.pos(2,0).size(1,1).weight(0.001,0).anchorCenter().fillHorizontal().insetsAll(0,2,0,2))
	add(selectFailedButton,	GBC.pos(3,0).size(1,1).weight(0.001,0).anchorCenter().fillHorizontal().insetsAll(0,2,0,0))
	add(selectStatus,		GBC.pos(4,0).size(1,1).weight(1,0).anchorCenter().fillHorizontal().insetsAll(0,8,0,4))
	add(scroll,				GBC.pos(0,1).size(5,1).weight(1,1).anchorCenter().fillBoth().insetsAll(4,0,0,0))
	...
	private class ListPanel extends JPanel with Scrollable {
		...
		*/
		def getScrollableUnitIncrement(visibleRect:Rectangle, orientation:Int, direction:Int):Int = {
			...
			if (direction < 0) {
				val component	= this.getComponentAt(visibleRect.x, visibleRect.y - 2)
				val	visible		= visibleRect.y
				val	target		= if (component != null) component.getY else 0
				visible - target
			}
			else {
				val component	= this.getComponentAt(visibleRect.x, visibleRect.y + visibleRect.height)
