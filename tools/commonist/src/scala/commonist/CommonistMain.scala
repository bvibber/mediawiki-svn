...
class CommonistMain {
	private val settingsDir	= HOME / ".commonist"
	settingsDir.mkdirs()
	INFO("settings directory: " + settingsDir)
	
	private val projectDir	= PWD / "etc"
	
	val programIcon		= null
	val programHeading	= "The Commonist " + Constants.VERSION
	
	private val loader		= new Loader(settingsDir, projectDir, "/etc/")
	...
	private def loadMessages(language:String) {
		val defaultURL	= loader resourceURL "messages.properties" getOrError "cannot load messages.properties"
