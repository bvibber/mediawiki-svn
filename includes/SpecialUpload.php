	...
	function internalProcessUpload( &$resultDetails ) {
		...
		if ( ! $this->mIgnoreWarning ) {
			...
			if ( !$this->mDestWarningAck ) {
				$warning .= self::getExistsWarning( $this->mLocalFile ); // @todo: when try upload an existing file through ApiUploadFile.php from ApiEditVodafone branch a parse error ocurred.
			}
			if( $warning != '' ) {
				/**
