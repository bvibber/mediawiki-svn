...
(function($){$.ArticleAssessment={'config':{'authtoken':'','userID':'','pageID':wgArticleId,'revID':wgCurRevisionId},'messages':{},'settings':{'endpoint':wgScriptPath+'/api.php?','fieldMessages':['wellsourced','neutrality','completeness','readability'],'fieldHintSuffix':'-tooltip','fieldPrefix':'articleassessment-rating-','fieldHTML':'<div class="field-wrapper"> \
   ...
   </div>','structureHTML':'<div class="article-assessment-wrapper"> \
   ...
   </div>','ratingHTML':'<div class="article-assessment-rating"> \
...
$.fn.stars=function(){var args=Array.prototype.slice.call(arguments);var out=$(this);$(this).each(function(){var context=$(this).data('stars-context');if(typeof context=='undefined'||context==null){context=$.extend({},{element:$(this),options:$.stars.defaults},$.stars);}
if(args.length>0){if(typeof args[0]=='object'){context.options=$.extend({},context.options,args[0]);$.stars.create.call(context);}else if(typeof args[0]=='string'){var funcName=args[0];out=$.stars[funcName].call(context,args.slice(1))||out;}}else{$.stars.create.call(context);}
$(this).data('stars-context',context);});return out;};})(jQuery);(function($){function Tipsy(element,options){this.$element=$(element);this.options=options;this.enabled=true;this.fixTitle();}
