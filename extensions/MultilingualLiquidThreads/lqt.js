...
// 			liquidThreads.addQuoteButton(toolbar);
	...
	'handleAJAXSave' : function( e ) {
		...
		var postlang = editform.find('#wpPostlang').val(); // Source language of posted article 
...
//			var threadID = post.data('thread-id'); Unused, but useful
		...
		if ( type == 'reply' ) {			
			liquidThreads.doReply( replyThread, text, postlang, summary,
						doneCallback, bump );
			
			e.preventDefault();
		} else if ( type == 'talkpage_new_thread' ) {
			liquidThreads.doNewThread( wgPageName, subject, text, postlang, summary,
					doneCallback, bump );
	...
	'doNewThread' : function( talkpage, subject, text, postlang, summary, callback, bump ) {
		liquidThreads.getToken(
			function(token) {
				var newTopicParams =
				{
					...
					'subject' : subject,
					'text' : text,
					'postlang' : postlang, // Source language of posted article
					'token' : token,
					'format' : 'json',
					'render' : '1',
					'reason' : summary,
	...
	'doReply' : function( thread, text, postlang, summary, callback, bump ) {
		liquidThreads.getToken(
			function(token) {
				var replyParams =
				{
