...
$pages.children().hide();$pages.find('*[rel='+selected+']').show();$index.children().removeClass('current');$selectedIndex.addClass('current');},build:function(context,config){var $tabs=$('<div />').addClass('tabs').appendTo(context.modules.$toolbar);if(wgNavigableTOCCollapseEnable){}
...
function drag(e){var mR=e.pageX-$('#wikiEditor-ui-left').offset().left;mR=$('#wikiEditor-ui-left').width()-mR;if(mR<26||mR>$('#wikiEditor-ui-left').width()-250)return false;$('#wikiEditor-ui-left').css('marginRight',mR+'px');$('#wikiEditor-ui-right').css('width',mR+'px');return false;}
function stopDrag(e){$().unbind('mousemove',drag).unbind('mouseup',stopDrag);context.modules.$toc.find('div').autoEllipse({'position':'right','tooltip':true});var mR=e.pageX-$('#wikiEditor-ui-bottom').offset().left;mR=$('#wikiEditor-ui-bottom').width()-mR;if(mR<50&&wgNavigableTOCCollapseEnable){$.wikiEditor.modules.toc.fn.collapse(context);}else{$('#wikiEditor-ui-left').trigger('mouseup');$('#wikiEditor-ui-right').data('openWidth',$('#wikiEditor-ui-right').width()+'px');$.cookie('wikiEditor-'+context.instance+'-toc-width',$('#wikiEditor-ui-right').width()+'px');}
return false;}
function buildResizeControls(){var $resizeControlVertical=$('<div />').attr('id','wikiEditor-ui-toc-resize-vertical').bind('mousedown',function(){$('#wikiEditor-ui-toc').data('openWidth',$('#wikiEditor-ui-toc').width());$().bind('mousemove',drag).bind('mouseup',stopDrag);});var $collapseControl=$('<div />').addClass('tab').addClass('tab-toc').append('<a href="#"></a>');if($.cookie('wikiEditor-'+context.instance+'-toc-width')!='1px'){$collapseControl.bind('click',function(){$.wikiEditor.modules.toc.fn.collapse(context);}).find('a').text('Hide Contents');}else{$collapseControl.bind('click',function(){$.wikiEditor.modules.toc.fn.expand(context);}).find('a').text('Show Contents');}
$collapseControl.insertBefore(context.modules.$toc);if(!$('#wikiEditor-ui-toc').data('openWidth')){$('#wikiEditor-ui-toc').data('openWidth',context.initialWidth=='1px'?$.wikiEditor.modules.toc.defaultWidth:context.initialWidth);}
