...
context.modules.$toolbar.find(selector).remove();}}},fn:{create:function(context,config){if('$toolbar'in context.modules){return;}
context.modules.$toolbar=$('<div></div>').addClass('wikiEditor-ui-toolbar');$.wikiEditor.modules.toolbar.fn.build(context,config);context.$ui.find('.wikiEditor-ui-top').append(context.modules.$toolbar);},performAction:function(context,action){switch(action.type){case'encapsulate':var parts={'pre':'','peri':'','post':''};for(part in parts){if(part+'Msg'in action.options){parts[part]=gM(action.options[part+'Msg'],(action.options[part]||null));}else{parts[part]=(action.options[part]||'')}}
...
break;case'select':if('list'in tool&&$(this).val()in tool.list){$.wikiEditor.modules.toolbar.fn.performAction(context,tool.list[$(this).val()].action);}
...
for(group in section.groups){var $group=$('<div></div>').attr('class','group').attr('rel',group).appendTo($section);if(objHasMsg(section.groups[group],'label')){$group.append($('<div></div>').attr('class','label').text(msg(section.groups[group],'label')))}
...
var $section=$('<div></div>').attr('rel',section).addClass('section loading').addClass('section-'+config[section].type).addClass('section-'+config[section].type+'-'+section).attr('id',sectionId).append($('<div></div>').addClass('spinner').text(gM('edittoolbar-loading'))).appendTo($sections);var current=false;if($.cookie(sectionCookie)==sectionId){$section.attr('style','display:block');current=true;}
sectionQueue[sectionQueue.length]={'$section':$section,'tools':config[section],'id':sectionId};$tabs.append($('<span></span>').attr('class','tab').attr('rel',section).append($('<a></a>').text(config[section].label||gM(config[section].labelMsg)).attr('href','#').addClass(current?'current':null).data('$section',$section).data('sectionCookie',sectionCookie).click(function(){var $section=$(this).data('$section');$(this).blur();var show=$section.css('display')=='none';$section.parent().children().hide();$(this).parent().parent().find('a').removeClass('current');if(show){$section.show();$(this).addClass('current');}
$.cookie($(this).data('sectionCookie'),show?$section.attr('id'):null);return false;})));}
$.eachAsync(sectionQueue,{bulk:0,loop:function(index,section){$.wikiEditor.modules.toolbar.fn.addSection(context,section.$section,section.tools,section.id);section.$section.removeClass('loading')}});}}};})(jQuery);