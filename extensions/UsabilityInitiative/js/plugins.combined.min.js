...
if(!($.browser.name in map[$('body').is('.rtl')?'rtl':'ltr'])){return $.wikiEditor.supported=true;}
var browser=map[$('body').is('.rtl')?'rtl':'ltr'][$.browser.name];for(var condition in browser){var op=browser[condition][0];var val=browser[condition][1];if(typeof val=='string'){if(!(eval('$.browser.version'+op+'"'+val+'"'))){return $.wikiEditor.supported=false;}}else if(typeof val=='number'){if(!(eval('$.browser.versionNumber'+op+val))){return $.wikiEditor.supported=false;}}}
...
context.history.push({'html':newHTML});while(context.history.length>10){context.history.shift();}}
return true;},'paste':function(event){context.$content.find(':not(.wikiEditor)').addClass('wikiEditor');context.$content.addClass('pasting');setTimeout(function(){var $selection=context.$content.find(':not(.wikiEditor)');while($selection.length&&$selection.length>0){var $currentElement=$selection.eq(0);while(!$currentElement.parent().is('body')&&!$currentElement.parent().is('.wikiEditor')){$currentElement=$currentElement.parent();}
if($currentElement.is('br')){$currentElement.addClass('wikiEditor');}else{$('<p></p>').text($currentElement.text()).addClass('wikiEditor').insertAfter($currentElement);$currentElement.remove();}
...
t=t.next();}
context.$content.removeClass('pasting');},0);return true;}};context.fn={'trigger':function(name,event){if(typeof event=='undefined'){event={'type':'custom'};}
...
var sc=selection.getRangeAt(0).startContainer,ec=selection.getRangeAt(0).endContainer;var so=selection.getRangeAt(0).startOffset,eo=selection.getRangeAt(0).endOffset;if(sc.nodeName=='BODY'){var n=body.firstChild;for(var i=0;i<so-1&&n;i++){n=n.nextSibling;}
sc=n;so=0;}
if(ec.nodeName=='BODY'){var n=body.firstChild;for(var i=0;i<eo-1&&n;i++){n=n.nextSibling;}
ec=n;eo=0;}
