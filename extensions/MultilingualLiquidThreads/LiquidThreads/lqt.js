...
var liquidThreads = {
	...
	'handleReplyLink' : function(e) {
		...
		if( liquidThreads.currentReplyThread != null ) {
			$j('.lqt-reply-form').fadeOut('slow', function() { $j(this).empty(); } );
			liquidThreads.currentSourceThread = null;
		}
	...
	'handleSourceLink' : function(e) {
		...
		if ( thread_id == liquidThreads.currentSourceThread ) {
			$j('.lqt-source-form').fadeOut('slow', function() { $j(this).empty(); } );
			
			liquidThreads.currentSourceThread = null;
			return;
		}

		var query = '&lqt_method=source&lqt_operand='+thread_id;

		var container = $j(target).closest('.lqt_thread')[0];
		var sourceDiv = $j(container).find('.lqt-source-form')[0];

		if( liquidThreads.currentSourceThread != null ) {
			$j('.lqt-source-form').fadeOut('slow', function() { $j(this).empty(); } );
			liquidThreads.currentSourceThread = null;
		}

		liquidThreads.showSourceForm( query, sourceDiv, e.preload );
		liquidThreads.currentSourceThread = thread_id;
	},
	...
	'showSourceForm' : function(query, container, preload) {
		...
		var finishShow = function() {
			...
			//scroll down
			if ( scrollOffset > $j(window).scrollTop() ) {
		...
		var finishSetup = function() {
			...
			var cancelButton = $j(container).find('#mw-source-cancel');
	...
	'setupThread' : function(threadContainer) {
		...
		// setup source link
		var sourceLinks = $j(threadContainer).find( '.lqt-command-source' );

		if ( sourceLinks.length ) {
			var sourceLink = sourceLinks.find('a');

			sourceLink.click( liquidThreads.handleSourceLink );
		}
