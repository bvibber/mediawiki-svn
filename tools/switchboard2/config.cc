...
configuration_loader::configuration_loader()
	: logger(log4cxx::Logger::getLogger("config"))
{
}
...
bool
configuration_loader::f_server_type(
		std::vector<std::string> &fields,
		config &newconf)
{
	if (fields.size() != 2) {
		LOG4CXX_ERROR(logger,
			format("\"%s\", line %d: usage: server-type <sjs|apache>\n")
			% file_ % lineno_);
		return false;
	}

	if (fields[1] == "sjs")
		newconf.servtype = serv_sjs;
	else if (fields[1] == "apache")
		newconf.servtype = serv_apache;
	else {
		LOG4CXX_ERROR(logger,
			format("\"%s\", line %d: usage: server-type <sjs|apache>\n")
			% file_ % lineno_);
