%define _basedir /opt/TSperl
%include Solaris.inc

Name:		TSperl-Mail-SPF-Query
Summary:	Mail::SPF::Query module for Perl
Version:	1.999.1
Source:		http://search.cpan.org/CPAN/authors/id/J/JM/JMEHNLE/mail-spf-query/Mail-SPF-Query-%{version}.tar.gz

SUNW_BaseDir:	%{_basedir}
BuildRoot:	%{_tmppath}/%{name}-%{version}-build
%include default-depend.inc

Requires: TSperl
Requires: TSperl-Net-CIDR-Lite
Requires: TSperl-Sys-Hostname-Long
BuildRequires: TSperl
BuildRequires: TSperl-Net-CIDR-Lite
BuildRequires: TSperl-Sys-Hostname-Long

%prep
%setup -q -n Mail-SPF-Query-%version

%build

/opt/TSperl/bin/perl Makefile.PL INSTALLDIRS=vendor
make

%install
rm -rf $RPM_BUILD_ROOT

make DESTDIR=$RPM_BUILD_ROOT install
rm $RPM_BUILD_ROOT%{_libdir}/5.8/perllocal.pod
rmdir $RPM_BUILD_ROOT%{_libdir}/5.8
find $RPM_BUILD_ROOT%{_libdir} -name .packlist -exec rm {} +

rm -f $RPM_BUILD_ROOT%{_bindir}/spfd
rm -f $RPM_BUILD_ROOT%{_bindir}/spfquery
rmdir $RPM_BUILD_ROOT%{_bindir}
rm -f $RPM_BUILD_ROOT%{_mandir}/man1/spfd.1
rm -f $RPM_BUILD_ROOT%{_mandir}/man1/spfquery.1
rmdir $RPM_BUILD_ROOT%{_mandir}/man1

%clean
rm -rf $RPM_BUILD_ROOT

%files
%defattr (-, root, bin)
%dir %attr (0755, root, bin) %{_libdir}
%dir %attr (0755, root, bin) %{_libdir}/vendor_perl
%dir %attr (0755, root, bin) %{_libdir}/vendor_perl/5.8
%{_libdir}/vendor_perl/5.8/*
%dir %attr (0755, root, sys) %{_datadir}
%dir %attr (0755, root, bin) %{_mandir}
%dir %attr (0755, root, bin) %{_mandir}/man3
%{_mandir}/man3/*

%changelog
* Fri Jan 30 2009 - river@loreley.flyingparchment.org.uk
- Initial spec
