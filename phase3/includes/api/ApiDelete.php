...
/**#@+
 * ApiDelete::delete() return values
 */
define('DELETE_SUCCESS', 0);
define('DELETE_PERM', 1); // Permission denied
define('DELETE_BLOCKED', 2); // You are blocked
define('DELETE_READONLY', 3); // Wiki is in read-only mode
define('DELETE_BADTOKEN', 4); // Invalid token
define('DELETE_BADARTICLE', 5); // $this is not a valid Article
...
class ApiDelete extends ApiBase {
	 ...
	 * Maybe Article.php can be changed some day to use this function instead.
	...
	public static function delete(&$article, $token, &$reason = NULL)
	{
		...
		if(wfReadOnly())
			return DELETE_READONLY;

		// Check token
		if(!$wgUser->matchEditToken($token))
			return DELETE_BADTOKEN;
	...
	public function execute() {
		...
		switch($retval)
		{
			...
			case DELETE_PERM:  // If we get PERM, BLOCKED or READONLY that's weird, but it's possible
				$this->dieUsage('You don\'t have permission to rollback', 'permissiondenied');
			case DELETE_BLOCKED:
				$this->dieUsage('You have been blocked from editing', 'blocked');
			case DELETE_READONLY:
				$this->dieUsage('The wiki is in read-only mode', 'readonly');
		...
		// $retval has to be DELETE_SUCCESS if we get here
